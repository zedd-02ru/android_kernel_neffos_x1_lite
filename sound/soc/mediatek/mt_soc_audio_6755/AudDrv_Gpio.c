...
#ifdef CONFIG_MTK_SMARTPA_SUPPORT
int AudDrv_GPIO_I2S3_Select(int bEnable)
{
	int retval = 0;

	if (bEnable == 1) {
		if (aud_gpios[GPIO_I2S3_MODE1].gpio_prepare) {
			retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_I2S3_MODE1].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_I2S3_MODE1] pins\n");
		}
	} else {
		if (aud_gpios[GPIO_I2S3_MODE0].gpio_prepare) {
			retval = pinctrl_select_state(pinctrlaud, aud_gpios[GPIO_I2S3_MODE0].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_I2S3_MODE0] pins\n");
		}

...
/* [yanghuaying start] Trace bug 20366 */
#ifndef CONFIG_MTK_NET_LOGGING
#define CONFIG_MTK_NET_LOGGING
#endif
/* [yanghuaying end] */
...
/* [yanghuaying start] Trace bug 20366*/
static ssize_t rtnl_mutex_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	*buf = 0;
	if(rtnl_mutex.owner != NULL) {
		pr_err("show rtnl_mutex owner stack:\n");
		show_stack(rtnl_mutex.owner, NULL);
	} else {
		pr_err("rtnl_mutex has no owner\n");
	}
	return 0;
}
static DEVICE_ATTR(rtnl_mutex_info, S_IRUGO, rtnl_mutex_info_show, NULL );
/* [yanghuaying end] */
...
void __init rtnetlink_init(void)
{
	...
	/* [yanghuaying start] Trace bug 20366*/
	if (sysfs_create_file(kernel_kobj, &dev_attr_rtnl_mutex_info.attr) < 0)
		pr_err("create dev_attr_rtnl_mutex_info fail\n");
	/* [yanghuaying end] */

...
void mmc_rescan(struct work_struct *work)
{
	...
	for (i = 0; i < ARRAY_SIZE(freqs); i++) {
		if (!(try_result = mmc_rescan_try_freq(host, max(freqs[i], host->f_min))))
			break;
		if (freqs[i] <= host->f_min)
			break;
	}

	pr_err("%s: mmc_rescan_try_freq: try_result = %d\n", mmc_hostname(host), try_result);

	if (try_result) {
		msdc = mmc_priv(host);
		if (msdc != NULL) {
			if (msdc->hw->host_function == MSDC_EMMC) {
				msdc->hw->cmd_edge = MSDC_SMPL_RISING;
				pr_err("%s: host_function == MSDC_EMMC change cmd_edge = MSDC_SMPL_RISING\n", mmc_hostname(host));

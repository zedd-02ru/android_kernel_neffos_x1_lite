...
/*save sd card power regulator when sd card power should always on*/
struct regulator *sd_vmch_reg = NULL;
...
/* [liguanxiong start] use common function to identify pvt machine */
extern int get_pcb_version_voltage(void);
extern int get_board_config(void);
static int is_pvt_machine(void)
{
	int PCBvoltage  = 0;
	int ret = 0;

	if (get_board_config() == 1) {
		/* config C is thought to be same as pvt */
		return 1;
	}
	PCBvoltage = get_pcb_version_voltage();
	pr_err("mmc PCBvoltage = %d\n", PCBvoltage);

	//evt:0.9v;  dvt1/2:0.6v;  dvt3:1.2;  pvt:0v;
	if (PCBvoltage < 500) {
		ret = 1;
	}
	return ret;
...
int msdc_of_parse(struct mmc_host *mmc)
{
	...
	/*if sd card power vmch diff:must not always-on in pvt,but should always-on
	* in dvt machine.*/
	if(of_find_property(np, "sd_card_power_diff_pvt", &len) && !is_pvt_machine()) {
		/* vmch always-on is not set by default,we let it
		* always-on for non-pvt machine here.*/
		sd_vmch_reg = mmc->supply.vmmc;
		pr_err("mmc, machine is not pvt,let power always on\n");
	}

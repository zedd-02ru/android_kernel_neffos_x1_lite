...
//static atomic_t double_enable_gt5668;
atomic_t double_enable_gt5668 ;
...
MODULE_DEVICE_TABLE(of, gt1x_dt_match);
static struct i2c_driver tpd_i2c_driver = {
	.driver = {
			.of_match_table = of_match_ptr(gt1x_dt_match),
			.name = "ft5x0x",
		},
	.probe = tpd_i2c_probe,
...
static int of_get_gt1x_platform_data(struct device *dev)
{
    if (dev->of_node)
    {
        const struct of_device_id *match;
        match = of_match_device(of_match_ptr(gt1x_dt_match), dev);
        if (!match)
        {
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	s32 idx = 0;
  	...
  	GTP_ERROR("....tp904 enter probe...\n");
	gt1x_i2c_client = client;
	/* [lichenggang start] modify for META mode*/
	boot_mode = get_boot_mode();
	GTP_ERROR("....boot_mode=%d...\n",boot_mode);
	//META_BOOT=1
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		GTP_DEBUG("i2c functionality check error\n");
		return 0;
	}
	of_get_gt1x_platform_data(&client->dev);
    init_goodix_proc();
	if (gt1x_init()) {
		/* TP resolution == LCD resolution, no need to match resolution when initialized fail */
		gt1x_abs_x_max = 0;
		gt1x_abs_y_max = 0;
		GTP_ERROR("probe return...IC is ft8607...\n");
		tpd_probe_result = -1;
		return -1;
	}
...
#ifdef SUPPORT_GESTURE
	atomic_set(&double_enable_gt5668,0); 
...
#endif
	if(report_count==100){
		GTP_ERROR("tpd_down::x[%d]__y[%d]=[%d %d]\n", id,id,x,y);
...
static int tpd_event_handler(void *unused)
{
	...
	do {
		...
		if (tpd_eint_mode) {
			...
			GTP_ERROR("Polling coordinate mode!");
...
static int tpd_local_init(void)
{
	...
	GTP_ERROR("tpd_probe_result=%d\n",tpd_probe_result);
	if(tpd_probe_result == -1)
	{
		return -1;	
	}
...
{                           
    char pagesize[512];
    int len = 0;
	GTP_INFO("double tap enable is: %d\n", atomic_read(&double_enable_gt5668));
...
static ssize_t tp_double_write_func(struct file *file, const char __user *buffer, size_t count,loff_t *ppos)
{
	...
	if( atomic_read(&is_in_suspend) )
	{
		...
		//up(&suspend_sem);
		mutex_unlock(&gt_suspend_lock);
		return count;//can not save double_enable_gt5668 flag when is_in_suspend
	...
	if((ret == 0 )||(ret == 1))
	{
		atomic_set(&double_enable_gt5668,ret);
		in_suspend_gustrue_status = ret;
	}
...
{
	mutex_lock(&gt_suspend_lock);
	pmic_set_register_value(PMIC_LDO_VLDO28_EN_1,0);
...
static int __init tpd_driver_init(void)
{
	...
	GTP_ERROR("tp904 tpd_driver_init\n");
	tpd_get_dts_info();

...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    TPD_ERR("...tp905..TPD probe....\n");   
    if (client == NULL)
    {
        TPD_ERR("i2c client is NULL\n");
        return -1;
    }
    g_I2cClient = client;
    MsDrvInterfaceTouchDeviceSetIicDataRate(g_I2cClient, 100000); // 100 KHz
		...
		if (ret)
		{
			TPD_ERR("Could not set to 2800mv.\n");
		}
	#endif 
#endif 
...
static int smart_tpd_local_init(void)
{  
    TPD_ERR("tp905 TPD init device driver\n");
    ...
    {
        TPD_DMESG("tp905 Add error touch panel driver.\n");
...
static struct tpd_driver_t tpd_device_driver = {
    ...
    .tpd_local_init = smart_tpd_local_init,
    .suspend = tpd_suspend,
    .resume = tpd_resume,
#ifdef CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
    .attrs = {
...
{
    TPD_ERR("tp905 MStar touch panel driver init\n");
    ...
    {
        TPD_ERR("TPD add MStar TP driver failed\n");
    }else{
    	TPD_ERR("TPD add MStar TP driver success!\n");
    }
    return 0;
}
...
{
    TPD_ERR("tp905 MStar touch panel driver exit\n");

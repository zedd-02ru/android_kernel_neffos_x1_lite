...
#include "msg22xx_booyi_update_bin.h" // for MSG22xx
#include "msg22xx_djn_update_bin.h"
...
/*=============================================================*/
/* [linchao start] This function is used by msg21xx, so remove it
...
static s32 _DrvFwCtrlSelfParsePacket(u8 *pPacket, u16 nLength, SelfTouchInfo_t *pInfo) // for MSG21xxA/MSG22xx
{
    ...
    {
        if (g_InterruptCount == 0xFFFFFFFF)
        {
            g_InterruptCount = 0; // Reset count if overflow
            TPD_ERR("g_InterruptCount reset to 0\n");
        }	
...
/* [linchao] This function will be used when to read hw_info */
...
static s32 _DrvFwCtrlMsg22xxUpdateFirmwareBySwId(void) 
{
    ...
    if (_gIsUpdateInfoBlockFirst == 1)
    {
        if ((g_IsUpdateFirmware & 0x10) == 0x10)
        {
            ...
            TPD_ERR("nCrcInfoA = 0x%x, nCrcInfoB = 0x%x\n", nCrcInfoA, nCrcInfoB);
            ...
            if (nCrcInfoA == nCrcInfoB)
            {
                ...
                TPD_ERR("nCrcMainA = 0x%x, nCrcMainB = 0x%x\n", nCrcMainA, nCrcMainB);
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
    ...
    TPD_ERR("***tp905::firmware update %s() ***\n", __func__);
    TPD_ERR("*** firmware update::g_Msg22xxChipRevision = 0x%x ***\n", g_Msg22xxChipRevision);
    ...
    TPD_ERR("nCrcMainA=0x%x, nCrcInfoA=0x%x, nCrcMainB=0x%x, nCrcInfoB=0x%x\n", nCrcMainA, nCrcInfoA, nCrcMainB, nCrcInfoB);
    ...
    if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) // Case 1. Main Block:OK, Info Block:OK
    {
        ...
        else if (eSwId == MSG22XX_SW_ID_DJN || eSwId == MSG22XX_SW_ID_DJN_TP905)
        {
        ...
        {
            TPD_ERR("eSwId = 0x%x is an undefined SW ID.\n", eSwId);
        ...
        TPD_ERR("eSwId=0x%x, nMajor=%d, nMinor=%d, nUpdateBinMajor=%d, nUpdateBinMinor=%d\n", eSwId, nMajor, nMinor, nUpdateBinMajor, nUpdateBinMinor);
        TPD_ERR("nMinor=%d, nUpdateBinMinor=%d\n",nMinor, nUpdateBinMinor);
        //if (nUpdateBinMinor > nMinor)
			
        if (nUpdateBinMinor != nMinor)
        {
            if ((eSwId == MSG22XX_SW_ID_BOOYI) && (nUpdateBinMajor == MSG22XX_SW_ID_BOOYI))
            {
                for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                {
                    if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_booyi_update_bin[i*1024]), 1024);
                    }
                    else // i == 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_booyi_update_bin[i*1024]), 512);
                    }
                }
            }
			
            //else if ((eSwId == MSG22XX_SW_ID_DJN) && (nUpdateBinMajor == MSG22XX_SW_ID_DJN))
            else if (((eSwId == MSG22XX_SW_ID_DJN) || (eSwId ==MSG22XX_SW_ID_DJN_TP905)) 
						&& ((nUpdateBinMajor == MSG22XX_SW_ID_DJN) || (nUpdateBinMajor == MSG22XX_SW_ID_DJN_TP905)))
            {
                for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE+1); i ++)
                {
                    if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) // i < 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_djn_update_bin[i*1024]), 1024);
                    }
                    else // i == 48
                    {
                        _DrvFwCtrlStoreFirmwareData(&(msg22xx_djn_update_bin[i*1024]), 512);
                    }
                }
            }
            else
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    TPD_ERR("*** tp905::update_%s() _gUpdateRetryCount = %d ***\n", __func__, _gUpdateRetryCount);
    ...
    if (g_ChipType == CHIP_TYPE_MSG21XXA)   
    {
        ...
        TPD_ERR("This chip type (0x%x) does not support update firmware by sw id\n", g_ChipType);
        DrvPlatformLyrTouchDeviceResetHw(); 
        DrvPlatformLyrEnableFingerTouchReport();
        nRetVal = -1;
        return;
    ...
    TPD_ERR("*** update firmware by sw id result = %d ***\n", nRetVal); 
    if (nRetVal == 0)
    {
        TPD_ERR("tp905 update firmware by sw id success\n");
        DrvPlatformLyrTouchDeviceResetHw();
        DrvPlatformLyrEnableFingerTouchReport();
    ...
    else //nRetVal == -1
    {
        ...
        {
            TPD_ERR("_gUpdateRetryCount = %d\n", _gUpdateRetryCount);
            queue_work(_gUpdateFirmwareBySwIdWorkQueue, &_gUpdateFirmwareBySwIdWork);
        }
        else
        {
            TPD_ERR("update firmware by sw id failed\n");
            DrvPlatformLyrTouchDeviceResetHw();
            DrvPlatformLyrEnableFingerTouchReport();
            if (g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX)    
            {
...
static s32 _DrvFwCtrlUpdateFirmwareC33(u8 szFwData[][1024], EmemType_e eEmemType)
{
    ...
    // erase main
    //_DrvFwCtrlEraseEmemC33(EMEM_MAIN); [linchao edit] This function is used by msg21xx, so remove it
    mdelay(1000);
...
static s32 _DrvFwCtrlMsg22xxUpdateFirmware(u8 szFwData[][1024], EmemType_e eEmemType)
{
    ...
    TPD_ERR("*** g_Msg22xxChipRevision = 0x%x ***\n", g_Msg22xxChipRevision);
    ...
    for (nEraseCount = 0; nEraseCount < nMaxEraseTimes; nEraseCount ++)
    {
        if (eEmemType == EMEM_ALL) // 48KB + 512Byte
        {
            TPD_ERR("Erase all block %d times\n", nEraseCount);
            // Clear pce
            RegSetLByteValue(0x1618, 0x80);
            mdelay(100);
            // Chip erase
...
u8 DrvFwCtrlGetChipType(void)
{
    ...
    if (rc < 0)
    {
        TPD_ERR("*** DbBusEnterSerialDebugMode() failed, rc = %d ***\n", rc);
        return nChipType;
    }
    DbBusStopMCU();
    DbBusIICUseBus();
    ...
    if (nChargerStatus) // charger plug in
    {
        if (_gChargerPlugIn == 0 || g_ForceUpdate == 1)
        {
            ...
            while (i < 5)
            {
                ...
                {
                    _gChargerPlugIn = 1;
                    TPD_ERR(&g_I2cClient->dev, "Update status for charger plug in success.\n");
                    break;
                }
            ...
            if (i == 5)
            {
                TPD_ERR(&g_I2cClient->dev, "Update status for charger plug in failed, rc = %d\n", rc);
            }
    ...
    else  // charger plug out
    {
        if (_gChargerPlugIn == 1 || g_ForceUpdate == 1)
        {
            ...
            while (i < 5)
            {
                ...
                {
                    _gChargerPlugIn = 0;
                    TPD_ERR(&g_I2cClient->dev, "Update status for charger plug out success.\n");
                    break;
                }
            ...
            if (i == 5)
            {
                TPD_ERR(&g_I2cClient->dev, "Update status for charger plug out failed, rc = %d\n", rc);
            }
            g_ForceUpdate = 0; // Clear flag after force update charger status
        }
    }	
...
void _DrvFwCtrlSelfHandleFingerTouch(void) // for MSG21xxA/MSG22xx
{
    ...
    if (_gIsDisableFinagerTouch == 1)
    {
        TPD_ERR("Skip finger touch for handling get firmware info or change firmware mode\n");
        return;
    }
    ...
    { 	
        ...
        else if (g_FirmwareMode == FIRMWARE_MODE_RAW_DATA_MODE)
        {
            ...
            if (g_SelfFirmwareInfo.nLogModePacketHeader != 0x62)
            {
                TPD_ERR("WRONG RAW DATA MODE HEADER : 0x%x\n", g_SelfFirmwareInfo.nLogModePacketHeader);
                goto TouchHandleEnd;		
            }
            nReportPacketLength = g_SelfFirmwareInfo.nLogModePacketLength;
            pPacket = g_LogModePacket;
    ...
    if (rc < 0)
    {
        TPD_ERR("I2C read packet data failed, rc = %d\n", rc);
        goto TouchHandleEnd;		
    }
#endif
    ...
    if (0 == _DrvFwCtrlSelfParsePacket(pPacket, nReportPacketLength, &tInfo))
    {
        ...
        if (g_IsEnableReportRate == 1)
        {
            if (g_ValidTouchCount == 0xFFFFFFFF)
            {

...
* Static variables
*******************************************************************************/
extern int TP_IS_DJN;
extern int TP_IS_TIMA;
static int TP_TIANMA_NEED_REPAIR=0;/* [lichenggang] add for tianma rework */
static int TP_DJN_NEED_REPAIR=0;/* [lichenggang] add for djn rework */

static unsigned char CTPM_FW[] =
{
    #include "FT8607_TP902_TIANMA_app.i"
};
...
/* [lichenggang start] add for panel rework*/
static unsigned char CTPM_FW_TIANMA_REPAIR[] =
{
    #include "FT8607_TP902_TIANMA_REPAIR_app.i"
};
static unsigned char CTPM_FW_DJN_REPAIR[] =
{
    #include "FT8607_TP902_DJN_REPAIR_app.i"
};
...
{
    #include "FT8607_TP902_DJN_app.i"
};
...
{
    #include "FT8607_Pramboot_V0.3_20160723.i"
...
int  fts_8607_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    FTS_DEBUG("8607 dw_lenth= %d",dw_lenth);
    if (dw_lenth > 0x10000 || dw_lenth ==0)
    {
...
int fts_ctpm_fw_upgrade_with_app_file(struct i2c_client *client, char *firmware_name)
{
    ...
    else if ((fts_updateinfo_curr.CHIP_ID==0x86))
    {
        ...
        //change fts_8606_writepram to fts_8607_writepram
        i_ret = fts_8607_writepram(client, aucFW_PRAM_BOOT, sizeof(aucFW_PRAM_BOOT));
        FTS_DEBUG("i_ret=%d\n",i_ret);
        if (i_ret != 0)
        {
            dev_err(&client->dev, "%s:fts_8607_writepram failed. err.\n",__func__);
            return -EIO;
        }

        i_ret =  fts_8607_ctpm_fw_upgrade(client, pbt_buf, fwsize);
    }
...
int fts_ctpm_get_i_file_ver_auto(u8 vendor_id)
{
    ...
    if ((vendor_id == FTS_VENDOR_ID_TIANMA) ||(vendor_id == FTS_VENDOR_ID_DJN))
    {
        /* [lichenggang start] add for panel rework*/
        if(TP_IS_DJN==1){
            if(TP_DJN_NEED_REPAIR){
                pr_err("%s:djn need repair\n",__func__);
                ui_sz = sizeof(CTPM_FW_DJN_REPAIR);
            }else{
                pr_err("%s:djn not need repair\n",__func__);
                ui_sz = sizeof(CTPM_FW_DJN);
            }
        }else{
            if(TP_TIANMA_NEED_REPAIR==1){
                pr_err("%s:tianma need repair\n",__func__);
                ui_sz = sizeof(CTPM_FW_TIANMA_REPAIR);
            }else{
            	pr_err("%s:tianma not need repair\n",__func__);
        ...
        if (fts_updateinfo_curr.CHIP_ID==FTS_CHIP_ID){
            ...
            if(TP_IS_DJN==1){
                if(TP_DJN_NEED_REPAIR){
                    return CTPM_FW_DJN_REPAIR[0x10e];
                }else{
                    return CTPM_FW_DJN[0x10e];
                }
            }else{
                if(TP_TIANMA_NEED_REPAIR==1){
                    return CTPM_FW_TIANMA_REPAIR[0x10e];
                }else{
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
    ...
    */
    ...
    else if (fts_updateinfo_curr.CHIP_ID==0x86)
    {
        ...
        if(TP_IS_DJN==1){
            pbt_buf=CTPM_FW_DJN;
            firmware_length=sizeof(CTPM_FW_DJN);
        }else{
        ...
        i_ret =  fts_8607_ctpm_fw_upgrade(client, pbt_buf, firmware_length);
        if (i_ret != 0)
        {
            dev_err(&client->dev, "[FTS] upgrade failed. err=%d.\n", i_ret);
        }
...
int fts_ctpm_fw_upgrade_with_i_file_auto(struct i2c_client *client,u8 vendor_id)
{
    ...
    if ((vendor_id == FTS_VENDOR_ID_TIANMA)||(vendor_id == FTS_VENDOR_ID_DJN))
    {
        /* [lichenggang start] add for tianma rework*/
        if(TP_IS_DJN==1){
            if(TP_DJN_NEED_REPAIR){
                pr_err("%s:get djn repair firmware file\n",__func__);
                fw_len = sizeof(CTPM_FW_DJN_REPAIR);
                pbt_buf = CTPM_FW_DJN_REPAIR;
            }else{
                pr_err("%s:get djn normal firmware file\n",__func__);
                fw_len = sizeof(CTPM_FW_DJN);
                pbt_buf = CTPM_FW_DJN;
    }
        }else{
            if(TP_TIANMA_NEED_REPAIR){
                pr_err("%s:get tianma repair firmware file\n",__func__);
                fw_len = sizeof(CTPM_FW_TIANMA_REPAIR);
                pbt_buf = CTPM_FW_TIANMA_REPAIR;
            }else{
                pr_err("%s:get tianma normal firmware file\n",__func__);
    ...
    */
    ...
    else if ((fts_updateinfo_curr.CHIP_ID==0x87))
    {
        ...
        //pbt_buf = CTPM_FW;
        /*call the upgrade function*/
        TPD_ERR("tp903 enter this function\n");
        i_ret = fts_8716_writepram(client, aucFW_PRAM_BOOT, sizeof(aucFW_PRAM_BOOT));
        if (i_ret != 0)
        {
            dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    do
    {
        ...
        /* [lichenggang start] modify for panel rework*/
        if(uc_tp_fm_ver>=0x2D){
            if(TP_IS_DJN){
                TPD_ERR("%s:tp is need rework_djn\n",__func__);
                TP_DJN_NEED_REPAIR=1;
            }else{
                TPD_ERR("%s:tp is need rework_tianma\n",__func__);
                TP_TIANMA_NEED_REPAIR=1;
            }
        }
        /* [lichenggang end] */
        if ((uc_tp_fm_ver > 0x50) || (uc_tp_fm_ver == 0))
        {
            i_ret = ft8607_get_rom_pram_id(client, reg_val);
            if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1)
            {
                ...
                for (i=0; i<50; i++)
                {
                    ...
                    i_ret = ft8607_get_rom_pram_id(client, reg_val);
                    TPD_ERR("[FTS_UPGRADE]::read rom id : ID1 = 0x%x,ID2 = 0x%x:\n",reg_val[0], reg_val[1]);
                    if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 && reg_val[1] == 0x07)
                    {
                        bUpgradeFlag = true;
                        TPD_ERR("[FTS_UPGRADE]: << %s >> : Run in Rom!!\n",__func__);
                        break;
                    }
                }
            }
        ...
        /* [lichenggang start] modify for DJN TP*/
        if(uc_tp_vendor_id == FTS_VENDOR_ID_DJN_ORIG){
            uc_tp_vendor_id = FTS_VENDOR_ID_DJN;
            printk("...tp is DJN...\n");
        }
        /* [lichenggang end] */

        if ((uc_tp_vendor_id == FTS_VENDOR_ID_TIANMA) || (uc_tp_vendor_id == FTS_VENDOR_ID_DJN))
        {
#ifdef  BOOT_UPGRADE_AUTO
            ...
            if (((uc_tp_fm_ver < uc_host_fm_ver) && uc_host_fm_ver != 0) || (uc_tp_fm_ver > 0x3F))// force upgrade
            {
                bUpgradeFlag = true;
            }
            /* [lichenggang end] */
            if (bUpgradeFlag)
            {
                ...
                /* [lichenggang start] modify for firmware version read
                    if version==0,read again*/
                    if(uc_tp_fm_ver==0)
                    {
                        msleep(50);
                        fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
            ...
            //FT8607_FIRMWARE_VERSION=uc_tp_fm_ver;
#else

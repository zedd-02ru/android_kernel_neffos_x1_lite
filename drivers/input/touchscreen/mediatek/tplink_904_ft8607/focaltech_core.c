...
static int TP904_FTS_PROBE_OK=0;
int TP_IS_TIMA=0;
int TP_IS_DJN=0;
...
#define CHANGE_TP_THRESHOLD_ENABEL

#ifdef CHANGE_TP_THRESHOLD_ENABEL
static atomic_t tp_thresh_adjust_enable;
static int tp_thresh_value = 1;
#endif

/* [lichenggang start] add for gesture judge*/
...
//[lichenggang]add for booting mode
...
static ssize_t tp_thresh_adjust_enable_write_func(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if((ret==1)||(ret==0))
    {
        atomic_set(&tp_thresh_adjust_enable,1);
    }
    mutex_unlock(&ft_suspend_lock);
    return count;
}
...
static ssize_t tp_thresh_value_write_func(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    mutex_lock(&ft_suspend_lock);
    tp_thresh_value=ret;
    /* [lichenggang start] modify for 904 tp threshold adjust real-time*/
    if(1==atomic_read(&tp_thresh_adjust_enable)){
        if(usb_check_state==1){
            ...
            /* [lichenggang start] add for djn*/
            if(TP_IS_DJN){
                if(tp_thresh_value==0){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+11));
                }else if (tp_thresh_value==1){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+9));
                }else if(tp_thresh_value==2){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+7));//tp_thresh_value+8
                }
            }else{
                if(tp_thresh_value==0){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+13));
                }else if (tp_thresh_value==1){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+10));
                }else if(tp_thresh_value==2){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+8));
                }
            }
            /* [lichenggang end] */
            msleep(50);
            ...
            if(TP_IS_DJN){
                if(tp_thresh_value==0){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+8));//tp_thresh_value+10
                }else if (tp_thresh_value==1){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+6));//tp_thresh_value+7
                }else if(tp_thresh_value==2){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+4));//tp_thresh_value+5
                }
            }else{
                if(tp_thresh_value==0){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+10));
                }else if (tp_thresh_value==1){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+7));
                }else if(tp_thresh_value==2){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+5));
                }
            }
            /* [lichenggang end] */
            msleep(50);
...
#ifdef READ_DIFFDATA_ENABLE
/* [lichenggang start] modify for diffdata read*/
...
/* [lichenggang start] add for gesture*/
...
int fts_i2c_read(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen)
{
    ...
    /* [lichenggang start]change 128 to 1280 just for getting ic diffdata,because the max readlen must greater than 2*TX*RX */
            ...
            if (ret == 0) {
                TPD_DEBUG("%s: read touch data success, start report event \n", __func__);
                    if(pevent.au16_y[0] != 1350){
                        /* [lichenggang start] modify for vir-key handle*/
                        if( key_back_button==1 || key_menu_button==1 || key_home_button==1 ){
                            if(key_back_button==1){
                                input_report_key(tpd->dev, KEY_BACK, 0);
                                input_sync(tpd->dev);
                                key_back_button=0;
...
//extern u8 FT8607_FIRMWARE_VERSION;
ssize_t tp_ft8607_info_read_func(struct device *dev, struct device_attribute *attr, char *buf)
{
    unsigned char panel_type[] = "ft8607";

    unsigned char panel_ic[2][10] = {"tianma","djn"};
    unsigned char  driver_version[] = "1.0";
    u32 nLength = 0;
    unsigned char vendor = 0;
    u8 FT8607_FIRMWARE_VERSION=0;
    fts_read_reg(fts_i2c_client, 0xA6, &FT8607_FIRMWARE_VERSION);
...
static DEVICE_ATTR(tp_info, S_IRUGO, tp_ft8607_info_read_func, NULL);



void get_tp_id_by_aux(void)
{
    int ret = 0, data[4], ret_value = 0;
    int Channel = AUX_IN2_NTC;
    static int valid_id = -1;

    pr_err("enter get_tp_id_by_aux\n");
    ...
    pr_err("[lcd_auxadc_get_data(AUX_IN2_NTC)]: ret=%d, data[0]=%d, data[1]=%d\n", ret, data[0], data[1]);
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    TPD_ERR("tp902:i2c_client_FT->addr=%d\n",i2c_client->addr);
        ...
        if(usb_check_state==1){
            ...
            if(TP_IS_DJN){
                if(tp_thresh_value==0){
                    ...
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+6));
                }else if(tp_thresh_value==2){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+4));
                }    
            }else{
                if(tp_thresh_value==0){
                    fts_write_reg(i2c_client,0x80,(tp_thresh_value+10));
...
static int ft8607_tpd_local_init(void)
{
    int retval = -1;
...
int usb_check(int usb_state)
{
    ...
    /* [lichenggang start] modify for fts probe check*/
    if(TP904_FTS_PROBE_OK==0)
    {
        TPD_ERR("904 probe fail and return\n");
        return 0;
    }
    /* [lichenggang end] */
    if(usb_state ==1)
...
static struct tpd_driver_t ft8607_tpd_device_driver =
{
    .tpd_device_name = "FT5x0x",
    .tpd_local_init = ft8607_tpd_local_init,
    .suspend = tpd_suspend,
    .resume = tpd_resume,
    .attrs = {
        .attr = ft5x0x_attrs,
...
{
    TPD_ERR("904::MediaTek FT5x0x touch panel driver init\n");
    tpd_get_dts_info();
    if (tpd_driver_add(&ft8607_tpd_device_driver) < 0)
...
static void __exit tpd_driver_exit(void)
{
    ...
    tpd_driver_remove(&ft8607_tpd_device_driver);
}

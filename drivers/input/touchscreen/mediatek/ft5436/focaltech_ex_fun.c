...
static ssize_t fts_debug_write(struct file *filp, const char __user * buff, size_t count, loff_t * ppos)
{
    ...
    case PROC_UPGRADE:
/* [liliwen start] remove PROC_UPGRADE */
#if 0
        {
...
/* [liliwen start] Add for firmware update */
ssize_t firmware_update_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
    char fwname[FILE_NAME_LENGTH];
    u8 uc_tp_fm_ver = 0;
...
#endif
    if (fts_updatefun_curr.upgrade_with_app_bin_file)
        fts_updatefun_curr.upgrade_with_app_bin_file(fts_i2c_client, fwname);
    fts_i2c_read_reg(fts_i2c_client, FTS_REG_FW_VER, &uc_tp_fm_ver);
    pr_err("[FTS]uc_tp_fm_ver = 0x%x\n",uc_tp_fm_ver);
#if FTS_ESDCHECK_EN
    fts_esdcheck_switch(ENABLE);
#endif
    fts_irq_enable();
...
ssize_t firmware_update_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    int len = 0;
    int ret = 0;
    u8 firmware_version;
    char page[512];
    fts_i2c_read_reg(fts_i2c_client, FTS_REG_FW_VER, &firmware_version);
    pr_err("button_reverse is:0x%x\n",firmware_version);

...
/* [liliwen] Remove fts_updatefun */
//struct fts_upgrade_fun fts_updatefun;
struct workqueue_struct *touch_wq;
struct work_struct fw_update_work;
u8 *g_fw_file;
int g_fw_len;
/* [liliwen] add boot_mode */
...
static int fts_ctpm_check_fw_ver(struct i2c_client *client)
{
    ...
    /* [lichenggang start]modify firmware update logic*/
    printk("[UPGRADE]: uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x!!", uc_tp_fm_ver, uc_host_fm_ver);
    if (((uc_tp_fm_ver < uc_host_fm_ver) && uc_host_fm_ver != 0) || (uc_tp_fm_ver > 0x2F)){
        return 1;
    }else {
        return 0;
    }
...
void fts_ctpm_upgrade_init(void)
{
    ...
    /* [liliwen] get boot mode */
    boot_mode = get_boot_mode();

    printk("INIT::boot_mode = %d ............\n", boot_mode);

    /* [liliwen start] change upgrade */
    if (!((boot_mode == META_BOOT) || (boot_mode == FACTORY_BOOT) ||
        (boot_mode == RECOVERY_BOOT) || (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT))) {
        wake_lock_init(&upgrade_lock, WAKE_LOCK_SUSPEND, "tp_wakelock");

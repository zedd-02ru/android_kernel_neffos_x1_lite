...
static int fts_ft5x46_get_i_file(struct i2c_client *client, int fw_valid)
{
    ...
    FTS_INFO("[UPGRADE]tp vendor id:%x, FTS_VENDOR_ID:%02x %02x %02x", vendor_id, FTS_VENDOR_1_ID, FTS_VENDOR_2_ID, FTS_VENDOR_3_ID);
    ret = 0;
    /* [liliwen start] booyi old vendor_id upgrade */
    if (vendor_id == 0x59) {

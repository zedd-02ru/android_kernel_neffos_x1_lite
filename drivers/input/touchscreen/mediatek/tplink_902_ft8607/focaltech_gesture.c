...
/* [lichenggang start]optimize gesture*/
...
#define PIXEL_DISTANCE_WIDTH   87
#define PIXEL_DISTANCE_HEIGHT  87
...
static void SpecGestureTraceGet(UINT8 uiPointSum)
{
	...
	/* [lichenggang start]optimizing gesture data*/
	...
	memset(coordinate_report, 0, sizeof(coordinate_report));
	{
		...
		if((gesture_id != GESTURE_DOUBLELINE) && (gesture_id != GESTURE_CW_DOUBLELINE)){
			...
			for(i = 0;i < pointnum;i++){
				...
				/* [lichenggang start]optimizing gesture data*/
				if((coordinate_doblelion_1_x[i]==0) || (coordinate_doblelion_1_x[i]>TP_MAX_X_VALUE) || (coordinate_doblelion_1_y[i]==0) || (coordinate_doblelion_1_y[i]>TP_MAX_Y_VALUE))
				{
		...
		if(gesture != UnkownGestrue ){
			...
			memset(raw_data_buf, 0, sizeof(raw_data_buf));/* [lichenggang start]optimize gesture*/
			gesture_direction = 0;

...
ssize_t tp_ft8607_info_read_func(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    fts_read_reg(fts_i2c_client, 0xA6, &uc_tp_fm_ver);
...
static int TP902_FTS_PROBE_OK=0;
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int retval = TPD_OK;
	int reset_count = 0;
...
int usb_check(int usb_state)
{
	...
	if(TP902_FTS_PROBE_OK==0)
	{
		TPD_ERR("902 probe fail and return\n");
		return 0;
	}
	/* [lichenggang end] */
	if(usb_state ==1)
...
{
    TPD_ERR("902::MediaTek FT5x0x touch panel driver init\n");
    tpd_get_dts_info();

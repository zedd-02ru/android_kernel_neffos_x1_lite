...
static int TP903_FTS_PROBE_OK=0;
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	int retval = TPD_OK;
	int reset_count = 0;
	...
	/* [lichenggang start] modify for tp freq*/
...
reset_proc:
	...
	if (sysfs_create_file(kernel_kobj, &dev_attr_tp_info.attr)) {
			TPD_ERR("tp902::sysfs_create_file tp_info fail\n");
			return -ENODEV;
	}
#ifdef CHANGE_TP_THRESHOLD_ENABEL
...
int usb_check(int usb_state)
{
	...
	if(TP903_FTS_PROBE_OK==0)
	{
		TPD_ERR("903 probe fail and return\n");
		return 0;
	}
	/* [lichenggang end] */
	if(usb_state ==1)

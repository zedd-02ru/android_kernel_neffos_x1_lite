...
static ssize_t fts_keyconfig_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ssize_t num_read_chars = 0;
...
***********************************************************************/
static ssize_t fts_keyconfig_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    unsigned int state;
    ssize_t ret = -EINVAL;
    ...
    if (ret)
        return ret;
	button_reverse = state;
    //tpd_button_reverse(state);
    return count;
}
/************************************************************************
...
//static ssize_t fts_fwinfo_show(struct device *dev, struct device_attribute *attr, char *buf)
ssize_t tp_info_read_func(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
    ssize_t num_read_chars = 0;
    u8 fwver = 0,ret=0,vendor_id;
    u8 vendorid = 0;
	char page[512];
    unsigned char panel_ic[3][PANEL_INFO_MAX] = {{"tianma"}, {"truly"}, {"unknown"}};
	#ifdef CONFIG_TPLINK_PRODUCT_TP902
    unsigned char panel_type[3][PANEL_INFO_MAX] = {{"ft8607"}, {"ft8607"}, {"unknown"}};
	#endif
	#ifdef CONFIG_TPLINK_PRODUCT_TP903
	unsigned char panel_type[3][PANEL_INFO_MAX] = {{"ft8716"}, {"ft8716"}, {"unknown"}};
	#endif

    mutex_lock(&fts_input_dev->mutex);
	fts_read_reg(fts_i2c_client, FTS_REG_FW_VER, &fwver);
    pr_err("%s:addr=0x%x,FTS_REG_FW_VER=0x%x\n", __func__, fts_i2c_client->addr,fwver);
    if (fts_read_reg(fts_i2c_client, FTS_REG_FW_VER, &fwver) < 0 || fwver == 255){
        num_read_chars = snprintf(page, PAGE_SIZE,"get tp fw version fail!\n");
        goto error_return;
    }

	fts_read_reg(fts_i2c_client, FTS_REG_VENDOR_ID, &vendor_id);
    pr_err("%s:addr=0x%x,FTS_REG_VENDOR_ID=0x%02x\n", __func__, fts_i2c_client->addr,vendor_id);
    if (fts_read_reg(fts_i2c_client, FTS_REG_VENDOR_ID, &vendorid) < 0){
        num_read_chars = snprintf(page, PAGE_SIZE,"get tp vendor id fail!\n");
        goto error_return;
    }

    if ((vendorid == FTS_VENDOR_ID_TIANMA)||(vendorid == FTS_VENDOR_ID_TIANMA_ORIG)){
        vendorid= 0;
    }
    else if ((vendorid == FTS_VENDOR_ID_TRULY)||(vendorid == FTS_VENDOR_ID_TRULY_ORIG)){
        vendorid= 1;
    }
    else {
        vendorid= 2;
    ...
    num_read_chars = snprintf(page, PAGE_SIZE,
        "type:\t%s\n"
        "vendor:\t%s\n"
        "firmware_version:\t%d\n"
        "driver_version:\t%s\n",
        &panel_type[vendorid][0],
        &panel_ic[vendorid][0],
        fwver,
...
*   example:cat fw_info
*/
static DEVICE_ATTR(ftskeyconfig, S_IRUGO|S_IWUSR, fts_keyconfig_show, fts_keyconfig_store);
/****************************************/
/* sysfs */
/* get the version info
*   example:cat fw_info
*/
//static DEVICE_ATTR(ftsfwinfo, S_IRUGO|S_IWUSR, fts_fwinfo_show, NULL);
/****************************************/
/* sysfs */
/* get the fw version

...
static unsigned char CTPM_FW[] =
{
	...
	#ifdef CONFIG_TPLINK_PRODUCT_TP903
		#include "FT8716_TP903_TIANMA_app.i"
	#endif
};
...
{
	#ifdef CONFIG_TPLINK_PRODUCT_TP902
		#include "FT8607_TP902_TRULY_app.i"
	#endif

	#ifdef CONFIG_TPLINK_PRODUCT_TP903
	//	#include "FT8716_TP903_TRULY_app.i"
	#endif
...
int fts_ctpm_get_i_file_ver_auto(u8 vendor_id)
{
    ...
    else if (vendor_id == FTS_VENDOR_ID_TRULY)
    {
        ui_sz = sizeof(CTPM_FW_TRULY);
        if(ui_sz == 0)
            return 0x00;
        if (fts_updateinfo_curr.CHIP_ID==FTS_CHIP_ID)
            return CTPM_FW_TRULY[0x10e];
        else
            return CTPM_FW_TRULY[ui_sz - 2];
    }
...
int fts_ctpm_fw_upgrade_with_i_file_auto(struct i2c_client *client,u8 vendor_id)
{
    ...
    else if (vendor_id == FTS_VENDOR_ID_TRULY)
    {
        ...
        pbt_buf = CTPM_FW_TRULY;
    }
    if(fw_len == 0){
        FTS_DEBUG("%s:firmware fw_len is 0\n",__func__);
        return -1;
    }
    /*judge the fw that will be upgraded
    * if illegal, then stop upgrade and return.
    */
    FTS_DEBUG("%s:TP CHIP_ID is %02X\n",__func__, fts_updateinfo_curr.CHIP_ID);
    if ((fts_updateinfo_curr.CHIP_ID==0x11) ||(fts_updateinfo_curr.CHIP_ID==0x12) ||(fts_updateinfo_curr.CHIP_ID==0x13) ||(fts_updateinfo_curr.CHIP_ID==0x14)
        ||(fts_updateinfo_curr.CHIP_ID==0x55) ||(fts_updateinfo_curr.CHIP_ID==0x06) ||(fts_updateinfo_curr.CHIP_ID==0x0a) ||(fts_updateinfo_curr.CHIP_ID==0x08))
    {
        if (fw_len < 8 || fw_len > 32 * 1024)
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
    ...
    do
    {
        ...
        if ((uc_tp_fm_ver > 0x50) || (uc_tp_fm_ver == 0))
        {
            ...
            {
                uc_boot_vendor_id = ft8607_ctpm_VidFWid_get_from_boot(client);
                FTS_DEBUG("[FTS_UPGRADE]: << %s >> : uc_boot_vendor_id= 0x%x!\n", __func__, uc_boot_vendor_id);

                if ((uc_boot_vendor_id == FTS_VENDOR_ID_TIANMA)|| (uc_boot_vendor_id == FTS_VENDOR_ID_TRULY))
                {
                ...
                {
                    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : FW .i unmatched,stop upgrade\n", __func__);
                }
            }
#else
            i_ret = ft8607_get_rom_pram_id(client, reg_val);
            if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1)
            {
                ...
                for (i=0; i<50; i++)
                {
                    ...
                    i_ret = ft8607_get_rom_pram_id(client, reg_val);
                        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : read rom id : ID1 = 0x%x,ID2 = 0x%x:\n",__func__, reg_val[0], reg_val[1]);
                    if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1 && reg_val[1] == 0x07)
                    {
                        bUpgradeFlag = true;
                        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : Run in Rom!!\n",__func__);
                        break;
                    }
                }
            }
        ...
        FTS_DEBUG("[FTS_UPGRADE]: << %s >> : uc_tp_vendor_id = 0x%x\n", __func__, uc_tp_vendor_id);
        if (uc_tp_vendor_id == FTS_VENDOR_ID_TIANMA_ORIG)
        {
            uc_tp_vendor_id = FTS_VENDOR_ID_TIANMA;
			printk("...tp is tiama\n");
			firmware_id=0x90210000;
		} else if (uc_tp_vendor_id == FTS_VENDOR_ID_TRULY_ORIG)
		{
            uc_tp_vendor_id = FTS_VENDOR_ID_TRULY;
			printk("...tp is truly\n");
			firmware_id=0x90200000;
		}
		//uc_tp_vendor_id = FTS_VENDOR_ID_TRULY;
        if ((uc_tp_vendor_id == FTS_VENDOR_ID_TIANMA)|| (uc_tp_vendor_id == FTS_VENDOR_ID_TRULY))
        {
           ...
           // if (uc_tp_fm_ver != uc_host_fm_ver && uc_host_fm_ver != 0) // force upgrade
            
            if (uc_tp_fm_ver != uc_host_fm_ver) // force upgrade
            {
                bUpgradeFlag = true;
            }
#if (FTS_UPGRADE_PINGPANG_TEST || FTS_UPGRADE_ERROR_TEST)
			...
			//sprintf(manufacture_info,"0x%x",((uc_tp_fm_ver&0x000000ff)|firmware_id));
#else
            ...
            if (bUpgradeFlag)
            {
                ...
                {
                    fts_read_reg(client, FTS_REG_FW_VER, &uc_tp_fm_ver);
                    FTS_DEBUG("[FTS_UPGRADE]: << %s >> : upgrade to new version 0x%x\n", __func__,uc_tp_fm_ver);
                }
                else
                {

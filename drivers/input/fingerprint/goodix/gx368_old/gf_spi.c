...
#include <linux/time.h> //[jiangtingyu]

#define GF_SPIDEV_NAME     "goodix,fingerprint"
/*device name after register in charater*/
#define GF_DEV_NAME            "goodix_fp"
// [taoyancheng] Ajust input dev name compatible to oem keyboard.
#define	GF_INPUT_NAME	    "fp_oem_kpd"	/*"goodix_fp" */
...
/*GF input keys*/
/* [taoyancheng start] Report oem keys */
struct gf_key_map key_map[] =
{
//    {"POWER"       , KEY_POWER   , KEY_POWER          },
    {"HOME"        , KEY_HOME    , KEY_FP_CLICK       },
//    {"MENU"        , KEY_MENU    , KEY_MENU           },
//    {"BACK"        , KEY_BACK    , KEY_FP_CLICK       },
    {"UP"          , KEY_UP      , KEY_FP_UP          },
    {"DOWN"        , KEY_DOWN    , KEY_FP_DOWN        },
    {"LEFT"        , KEY_LEFT    , KEY_FP_LEFT        },
    {"RIGHT"       , KEY_RIGHT   , KEY_FP_RIGHT       },
    {"LONG_CLICK"  , KEY_F1      , KEY_FP_LONG_CLICK  },
    {"DOUBLE_CLICK", KEY_F2      , KEY_FP_DOUBLE_CLICK},
//    {"FORCE"       , KEY_F9      , KEY_F9             },
//    {"APP_SWITCH"  , KEY_F19     , KEY_F19            },
//    {"F3"          , KEY_F3      , KEY_F3             },
    {"SPEED_UP"    , KEY_RESERVED, KEY_FP_SPEED_UP    } // Add to inform WindowManager of using performence config
};
/* [taoyancheng end] */
/**************************debug******************************/
#define GF_DEBUG
...
static unsigned int gf_suspend_flag = 0;
...
#define GF_LIB_VERSION    "20170609"
#define GF_DRIVER_VERSION "v2.0<20161128>"
#define GF_GX368_CHIP_ID  0x00002202
    ...
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (!strcmp("CMD_MODULE_READY", buf)) {
        gf_power_on(gf_dev, old_gpio_config);
    } else if (!strcmp("CMD_MODULE_PREPARE", buf)) {
        gf_power_off(gf_dev, old_gpio_config);
    }
    return count;
}
...
/* [taoyancheng start] No func mt_spi_enable_clk in MT6755 */
#elif defined(MT6797)
    ...
    //mt_spi_enable_master_clk(gf_dev->spi); // [taoyancheng] Remove since it will prevent kernel from deepidle.
/* [taoyancheng end] */
#endif
...
#if 0 // [taoyancheng] Func not used
...
static long gf_ioctl(struct file *filp, unsigned int cmd,
                     unsigned long arg)
{
    int i; // [taoyancheng] Add for key remap
    ...
    case GF_IOC_SENDKEY:
        ...
        /* [taoyancheng start] remap report keys */
		for(i = 0; i< ARRAY_SIZE(key_map); i++) {
            if(key_map[i].val == gf_key.key){
                input_report_key(gf_dev->input, key_map[i].val_oem, gf_key.value);
                input_sync(gf_dev->input);
                break;
            }
        }
        if(i == ARRAY_SIZE(key_map)) {
            pr_warn("key %d not support yet", gf_key.key);
            retval = -EFAULT;
        }
        /* [taoyancheng end] */
        break;
...
static irqreturn_t gf_irq(int irq, void *handle)
{
    ...
    if(gf_suspend_flag == 1){
		pr_info("gf_suspend_flag is ----1-----. \n");
		return IRQ_HANDLED;
    	}
#ifdef GF_FASYNC
...
{
    /* [taoyancheng start] register oem keys */
    int i;
    __set_bit(EV_KEY, gf_dev->input->evbit);
    for(i = 0; i< ARRAY_SIZE(key_map); i++) {
        __set_bit(key_map[i].val_oem, gf_dev->input->keybit);
    }
    /* [taoyancheng end] */
...
/* [taoyancheng end] */
    ...
    if (gf_parse_dts(gf_dev, old_gpio_config))
        goto error;

    if (gf_power_on(gf_dev, old_gpio_config))
        goto error;

/* [taoyancheng] Remove this reset since gf_open already contains one
...
*/
    ...
    if ((status == 0) && (sysfs_create_file(kernel_kobj, &dev_attr_fp_info.attr) != 0)) {
        pr_warn("Failed to create fp_info.");
        goto creat_fp_info_failed;
    }

...
#define RT5081A_PMU_CHARGER_DRV_VERSION	"1.1.12_MTK"
	...
	bool en)
{
	...
	ret = rt5081a_pmu_reg_block_write(
		chg_data->chip,
...
out:
	/* Exit hidden mode */
	ret = rt5081a_enable_hidden_mode(chg_data, false);
	if (ret < 0)
...
static int rt5081a_run_aicl(struct rt5081a_pmu_charger_data *chg_data)
{
	int ret = 0;
...
static void rt5081a_aicl_work_handler(struct work_struct *work)
{
	int ret = 0;
	struct rt5081a_pmu_charger_data *chg_data =
		(struct rt5081a_pmu_charger_data *)container_of(work,
		struct rt5081a_pmu_charger_data, aicl_work);
...
static void rt5081a_chgdet_work_handler(struct work_struct *work)
{
	...
	rt5081a_set_usbsw_state(chg_data, RT5081A_USBSW_CHG);
...
static int rt5081a_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if (en) {
		...
		if (ret < 0 || !en_otg) {
			...
			/* Disable OTG */
			rt5081a_pmu_reg_clr_bit(chg_data->chip,
				RT5081A_PMU_REG_CHGCTRL1, RT5081A_MASK_OPA_MODE);
...
static int rt5081a_set_pep20_reset(struct charger_device *chg_dev)
{
	...
	ret = rt5081a_set_ichg(chg_dev, 512000);
	if (ret < 0)
		return ret;
...
static int rt5081a_enable_chg_type_det(struct charger_device *chg_dev, bool en)
{
	...
	if (en)
		rt5081a_chg_irq_clr_flag(chg_data,
			&chg_data->irq_flag[RT5081A_CHG_IRQIDX_QCIRQ],
			RT5081A_MASK_ATTACHI);
	else { /* TypeC detach */
		ret = rt5081a_enable_ilim(chg_data, true); /* Enable ILIM_EN */
		if (ret < 0)
	...
	/* No matter plug in/out, make usb switch to RT5081A */
	rt5081a_set_usbsw_state(chg_data, RT5081A_USBSW_CHG);
	ret = rt5081a_enable_chgdet_flow(chg_data, en);
	if (ret < 0)
...
static irqreturn_t rt5081a_pmu_ovpctrl_uvp_d_evt_irq_handler(int irq, void *data)
{
	...
	/* Check UVP_D_STAT */
	ret = rt5081a_pmu_reg_test_bit(
		chg_data->chip,
		RT5081A_PMU_REG_OVPCTRLSTAT,
		RT5081A_SHIFT_OVPCTRL_UVP_D_STAT,
		&uvp_d_stat
	);
	if (!uvp_d_stat) {
...
static int rt5081a_pmu_charger_probe(struct platform_device *pdev)
{
	...
	/* Init AICL workqueue */
	INIT_WORK(&chg_data->aicl_work, rt5081a_aicl_work_handler);
#ifndef CONFIG_TCPC_CLASS

...
#define RT5081A_PMU_CHARGER_DRV_VERSION	"1.1.16_MTK"

#if defined(CONFIG_MTK_EXTERNAL_CHARGER_TYPE_DETECT)
#define __RT5081A_CHARGER_TYPE_DETECT__
#endif
...
struct rt5081a_pmu_charger_data {
	...
	bool bc12_en;
	u32 hidden_mode_cnt;
#if !defined(CONFIG_TCPC_CLASS) && defined(__RT5081A_CHARGER_TYPE_DETECT__)
	...
	enum rt5081a_adc_sel adc_sel, int *adc_val)
{
	...
	/* Calculate ADC value */
	adc_result = (adc_data[0] * 256 + adc_data[1]) * rt5081a_adc_unit[adc_sel]
...
#ifdef __RT5081A_CHARGER_TYPE_DETECT__
	...
	bool force_pwr_rdy)
{
	...
	ret = __rt5081a_chgdet_handler(chg_data, force_pwr_rdy);
	mutex_unlock(&chg_data->chgdet_lock);
	return ret;
}
#endif /* __RT5081A_CHARGER_TYPE_DETECT__ */
...
static int rt5081a_enable_te(struct rt5081a_pmu_charger_data *chg_data, bool en)
{
	int ret = 0;
...
static int rt5081a_set_ieoc(struct rt5081a_pmu_charger_data *chg_data, u32 ieoc)
{
	int ret = 0;
...
static int _rt5081a_set_mivr(struct rt5081a_pmu_charger_data *chg_data, u32 uV)
{
	int ret = 0;
	u8 reg_mivr = 0;
...
static int __rt5081a_run_aicl(struct rt5081a_pmu_charger_data *chg_data)
{
	...
	/* Set AICR to max */
	ret = _rt5081a_set_aicr(chg_data, RT5081A_AICR_MAX);
	if (ret < 0)
		goto unlock_out;
...
#endif /* !CONFIG_TCPC_CLASS && __RT5081A_CHARGER_TYPE_DETECT__ */

static int _rt5081a_set_ichg(struct rt5081a_pmu_charger_data *chg_data, u32 uA)
{
	int ret = 0;
	u8 reg_ichg = 0;
	...
	if (uA < 900000) /* 900mA */
		ret = rt5081a_set_ieoc(chg_data,
			chg_data->chg_desc->ieoc + 100000);
	else
		ret = rt5081a_set_ieoc(chg_data, chg_data->chg_desc->ieoc);

	return ret;
}

static int _rt5081a_set_cv(struct rt5081a_pmu_charger_data *chg_data, u32 uV)
{
	int ret = 0;
	u8 reg_cv = 0;
...
static int _rt5081a_enable_safety_timer(struct rt5081a_pmu_charger_data *chg_data,
	bool en)
{
	int ret = 0;
...
static int _rt5081a_enable_hz(struct rt5081a_pmu_charger_data *chg_data, bool en)
{
	int ret = 0;
...
static int _rt5081a_set_ircmp_resistor(struct rt5081a_pmu_charger_data *chg_data,
	u32 uohm)
{
	int ret = 0;
	u8 reg_resistor = 0;
...
static int _rt5081a_set_ircmp_vclamp(struct rt5081a_pmu_charger_data *chg_data,
	u32 uV)
{
	int ret = 0;
	u8 reg_vclamp = 0;
...
static int rt5081a_enable_hz(struct mtk_charger_info *mchr_info, void *data)
{
	...
	ret = _rt5081a_enable_hz(chg_data, en);
...
static int rt5081a_run_aicl(struct mtk_charger_info *mchr_info, void *data)
{
	...
	if (!(chg_data->irq_flag[RT5081A_CHG_IRQIDX_CHGIRQ1] & RT5081A_MASK_CHG_MIVR)) {
	...
	rt5081a_chg_irq_clr_flag(chg_data, &chg_data->irq_flag[RT5081A_CHG_IRQIDX_CHGIRQ1],
		RT5081A_MASK_CHG_MIVR);
...
en_mivrirq:
	ret = rt5081a_pmu_reg_clr_bit(chg_data->chip,
		RT5081A_PMU_CHGMASK1, RT5081A_MASK_CHG_MIVRM);
...
static int rt5081a_set_ircmp_resistor(struct mtk_charger_info *mchr_info, void *data)
{
	...
	ret = _rt5081a_set_ircmp_resistor(chg_data, uV);
...
static int rt5081a_set_ircmp_vclamp(struct mtk_charger_info *mchr_info, void *data)
{
	...
	ret = _rt5081a_set_ircmp_vclamp(chg_data, uV);
...
static int rt5081a_enable_safety_timer(struct mtk_charger_info *mchr_info, void *data)
{
	...
	ret = _rt5081a_enable_safety_timer(chg_data, en);
...
static int rt5081a_enable_power_path(struct mtk_charger_info *mchr_info, void *data)
{
	...
	ret = _rt5081a_set_mivr(chg_data, mivr);
...
static int rt5081a_set_ichg(struct mtk_charger_info *mchr_info, void *data)
{
	...
	mutex_lock(&chg_data->ichg_access_lock);
	ret = _rt5081a_set_ichg(chg_data, uA);
	mutex_unlock(&chg_data->ichg_access_lock);
...
static int rt5081a_set_mivr(struct mtk_charger_info *mchr_info, void *data)
{
	...
	ret = _rt5081a_set_mivr(chg_data, uV);
	...
	void *data)
{
	...
	/* TypeC detach */
	if (!en) {
		ret = rt5081a_chgdet_handler(chg_data, false);
		return ret;
	}
...
static irqreturn_t rt5081a_pmu_chg_mivr_irq_handler(int irq, void *data)
{
	...
	rt5081a_chg_irq_set_flag(chg_data, &chg_data->irq_flag[RT5081A_CHG_IRQIDX_CHGIRQ1],
		RT5081A_MASK_CHG_MIVR);
...
static irqreturn_t rt5081a_pmu_attachi_irq_handler(int irq, void *data)
{
	...
	mutex_unlock(&chg_data->bc12_access_lock);
	rt5081a_chgdet_handler(chg_data, true);
#endif
	...
	struct rt5081a_pmu_charger_data *chg_data)
{
	...
	if (of_property_read_string(np, "charger_name",
		...
		chg_data->mchr_info.name = "rt5081a_charger";
	}
...
static int rt5081a_chg_init_setting(struct rt5081a_pmu_charger_data *chg_data)
{
	...
	ret = _rt5081a_set_ichg(chg_data, chg_desc->ichg);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: set ichg failed\n", __func__);

	ret = _rt5081a_set_aicr(chg_data, chg_desc->aicr);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: set aicr failed\n", __func__);

	ret = _rt5081a_set_mivr(chg_data, chg_desc->mivr);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: set mivr failed\n", __func__);

	ret = _rt5081a_set_cv(chg_data, chg_desc->cv);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: set voreg failed\n", __func__);

	ret = rt5081a_set_ieoc(chg_data, chg_desc->ieoc);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: set ieoc failed\n", __func__);

	ret = rt5081a_enable_te(chg_data, chg_desc->en_te);
	if (ret < 0)
	...
	ret = _rt5081a_enable_safety_timer(chg_data, true);
	if (ret < 0)
	...
	/* Disable HZ */
	ret = _rt5081a_enable_hz(chg_data, false);
	if (ret < 0)
		dev_err(chg_data->dev, "%s: disable hz failed\n", __func__);

	ret = _rt5081a_set_ircmp_resistor(chg_data, chg_desc->ircmp_resistor);
	if (ret < 0)
		dev_err(chg_data->dev,
			"%s: set IR compensation resistor failed\n", __func__);

	ret = _rt5081a_set_ircmp_vclamp(chg_data, chg_desc->ircmp_vclamp);
	if (ret < 0)
		dev_err(chg_data->dev,

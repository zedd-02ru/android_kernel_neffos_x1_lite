...
/* [liliwen start] Add lcd info for TP902/903/904 */
#define PANEL_DRIVER_VERSION "1.0.0"
#define PANEL_FIRMWARE_VERSION "1.0.0"
#define PANEL_INFO_MAX  64
/* [liliwen start] Get LCM adc vol */
#define AUX_IN2_NTC  (12)
#define TIANMA_VOL      (0)
#define DJN_VOL         (1)
/* [lichenggang start] get lcd info for tp910 */
#define R61350_VOL      (2)
#define HX8394F_VOL      (3)
/* [lichenggang end] */
#define UNKNOWN_VOL     (999)
#define VOL_THRESHOLD_MIN1 (0)
#define VOL_THRESHOLD_MAX1 (300)
#define VOL_THRESHOLD_MIN2 (700)
#define VOL_THRESHOLD_MAX2 (1100)

extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int* rawdata);

int mtkfb_get_lcd_id_by_aux(void)
{
    ...
    DISPMSG("[lcd_auxadc_get_data(AUX_IN2_NTC)]: ret=%d, data[0]=%d, data[1]=%d\n", ret, data[0], data[1]);
    ...
    DISPMSG("APtery output mV = %d\n", ret);
    /* [lichenggang start] add lcd info for 910 */
		...
		struct device_attribute *attr, char *buf)
{
	...
	/* [liliwen start] Add djn LCM info */
	unsigned char panel_ic[6][PANEL_INFO_MAX] = {{"ft8607"}, {"ft8607"}, {"ft8716"}, {"s61350"},{"hx8394f"},{"unknown"}};
	unsigned char panel_type[6][PANEL_INFO_MAX] = {{"tianma"}, {"djn"}, {"tianma"}, {"truly"},{"booyi"},{"unknown"}};

#if defined(CONFIG_TPLINK_PRODUCT_TP904) || defined(CONFIG_TPLINK_PRODUCT_TP910)
	ret = mtkfb_get_lcd_id_by_aux();
	if (ret == TIANMA_VOL) {
		vendor = 0;
	} else if (ret == DJN_VOL) {
		vendor = 1;
/* [lichenggang start] add lcd info for 910 */
	} else if (ret == R61350_VOL) {
		vendor = 3;
	} else if (ret == HX8394F_VOL) {
		vendor = 4;
	}
/* [lichenggang end] */
#elif defined(CONFIG_TPLINK_PRODUCT_TP902)
	vendor = 0;
...
/* [liliwen start] Add lcm_adc_vendor */
static ssize_t mtkfb_show_lcm_adc_vendor(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	unsigned char vendor = 0;
	int ret = 0;
	/* [lichenggang start]add for 910,for distinguish diff vendor*/
#if defined(CONFIG_TPLINK_PRODUCT_TP904) || defined(CONFIG_TPLINK_PRODUCT_TP910)
	ret = mtkfb_get_lcd_id_by_aux();
	if ((ret == TIANMA_VOL) || (ret==R61350_VOL)) {// for 904 is tianma,for 910 is truly
		vendor = 0;
	} else if ((ret == DJN_VOL) || (ret== HX8394F_VOL)){//for 904 is djn,for 910 is booyi
		vendor = 1;
	}
#else
	vendor = 0; // TP902, tianma
#endif
	/* [lichenggang end] */
...
/* [liliwen] Add lcm_adc_vendor */
static DEVICE_ATTR(lcm_adc_vendor, S_IRUGO, mtkfb_show_lcm_adc_vendor, NULL);
/* [liliwen end] */
...
static int mtkfb_probe(struct device *dev)
{
	...
	/* [liliwen] Add lcm_adc_vendor */
	if (sysfs_create_file(kernel_kobj, &dev_attr_lcm_adc_vendor.attr)) {
		DISPERR("sysfs_create_file lcm_adc_vendor fail\n");
		return -ENODEV;
	}

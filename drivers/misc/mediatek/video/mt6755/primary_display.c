...
bool is_in_fingerprint_wq(void){
	char task_comm[32];

	get_task_comm(task_comm, current);
	DISPERR("%s, current thread is %s\n", __func__, task_comm);
	if(strstr(task_comm, "kworker"))
	{
		return true;
	}
	return false;
...
//[jiangtingyu start] improve finger lightup speed
EXPORT_SYMBOL_GPL(primary_display_resume);
//[jiangtingyu end]

...
extern atomic_t double_enable; /* [jiangtingyu] disable esd-check in double-click wakeup mode */
...
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	while (1) {
		...
		/* [jiangtingyu start] disable esd-check in double-click wakeup mode */
		if(atomic_read(&double_enable) == 1){
			DISPERR("[ESD]skip esd check , while in double click wakeup mode");
		}else if (esd_check_enable) {
		/* [jiangtingyu end] */

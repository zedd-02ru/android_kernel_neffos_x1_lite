...
/* [lifeiping start] for reliability test */
...
static int mtktsbattery_get_trip_temp(struct thermal_zone_device *thermal, int trip,
				      unsigned long *temp)
{
	/* [lifeiping start] disable battery_sysrst function during realiability test */
	if (KAL_FALSE == get_Temp_Charging_Protect_Mode()) {
...
/* BUG(); */
		/* arch_reset(0,NULL); */
		/* [lifeiping start] modify sysrst methods */

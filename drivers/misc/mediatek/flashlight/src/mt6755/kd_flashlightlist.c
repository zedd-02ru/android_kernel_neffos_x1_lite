...
#ifdef USE_PART2
int getSensorDev(int sensorDevIndex)
{
	if (sensorDevIndex == 0)
		return e_CAMERA_MAIN_SENSOR;
	else if (sensorDevIndex == 1)
		return e_CAMERA_SUB_SENSOR;
	else if (sensorDevIndex == 2)
		return e_CAMERA_MAIN_2_SENSOR;
	logI("sensorDevIndex=%d is wrong", sensorDevIndex);
	return -1;
}

int getStrobeId(int strobeIndex)
{
	if (strobeIndex < 0 || strobeIndex > 1) {
		logI("strobeIndex=%d is wrong", strobeIndex);
		return -1;
	}
	return strobeIndex + 1;
}
#endif
...
int globalInit(void)
{
	...
	for (i = 0; i < e_Max_Sensor_Dev_Num; i++)
		for (j = 0; j < e_Max_Strobe_Num_Per_Dev; j++) {
			...
			sensorDevIndex = getSensorDev(i);
			strobeIndex = getStrobeId(j);
			g_strobePartId[i][j] = strobe_getPartId(sensorDevIndex, strobeIndex);
			#else

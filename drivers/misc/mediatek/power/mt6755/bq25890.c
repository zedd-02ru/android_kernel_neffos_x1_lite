...
void bq25890_set_batfet_disable(unsigned int val)
{
	unsigned int ret = 0;

	ret = bq25890_config_interface((unsigned char) (bq25890_CON9),
				       (unsigned char) (val),
				       (unsigned char) (FORCE_BATFET_MASK),
				       (unsigned char) (FORCE_BATFET__SHIFT)
	    );
}
...
void bq25890_dump_register(void)
{
	...
	if (Enable_BATDRV_LOG == BAT_LOG_CRTI) {	// [lidebiao] dump register value
		bq25890_ADC_start(1);

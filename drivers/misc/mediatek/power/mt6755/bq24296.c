...
int bq24296_read_byte(unsigned char cmd, unsigned char *returnData)
{
    ...
    new_client->ext_flag =
     ((new_client->ext_flag) & I2C_MASK_FLAG) | I2C_WR_FLAG | I2C_DIRECTION_FLAG| I2C_RS_FLAG;	// [lidebiao] Open repeat start;
                 ...
                 unsigned char SHIFT)
{
    ...
    battery_log(BAT_LOG_FULL, "[bq24296_read_interface] Reg[%x]=0x%x", RegNum, bq24296_reg);

    bq24296_reg &= (MASK << SHIFT);
    *val = (bq24296_reg >> SHIFT);

    battery_log(BAT_LOG_FULL, "[bq24296_read_interface] val=0x%x", *val);
                 ...
                 unsigned char SHIFT)
{
    ...
    ret = bq24296_read_byte(RegNum, &bq24296_reg);
    battery_log(BAT_LOG_FULL, "[bq24296_config_interface] Reg[%x]=0x%x", RegNum, bq24296_reg);

    bq24296_reg &= ~(MASK << SHIFT);
    bq24296_reg |= (val << SHIFT);

    ret = bq24296_write_byte(RegNum, bq24296_reg);
    battery_log(BAT_LOG_FULL, "[bq24296_config_interface] write Reg[%x]=0x%x", RegNum, bq24296_reg);
...
void bq24296_hw_component_detect(void)
{
    ...
    battery_log(BAT_LOG_CRTI, "[bq24296_hw_component_detect] exist=%d, Reg[0x03]=0x%x", g_bq24296_hw_exist, val);
}

int is_bq24296_exist(void)
{
    battery_log(BAT_LOG_CRTI, "[is_bq24296_exist] g_bq24296_hw_exist=%d", g_bq24296_hw_exist);
...
{
    battery_log(BAT_LOG_CRTI, "[show_bq24296_access] 0x%x", g_reg_value_bq24296);
    return sprintf(buf, "%u", g_reg_value_bq24296);
}
                 ...
                 const char *buf, size_t size)
{
    ...
    if (buf != NULL && size != 0) {
        battery_log(BAT_LOG_CRTI, "[store_bq24296_access] buf is %s and size is %zu", buf, size);
...
static int __init bq24296_subsys_init(void)
{
    ...
    if (ret) {
        battery_log(BAT_LOG_CRTI, "****[bq24296_init] Unable to device register(%d)", ret);
        return ret;
    }
    ret = platform_driver_register(&bq24296_user_space_driver);
    if (ret) {
        battery_log(BAT_LOG_CRTI, "****[bq24296_init] Unable to register driver (%d)", ret);
        return ret;
    }

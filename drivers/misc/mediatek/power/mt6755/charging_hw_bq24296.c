                 ...
                 const unsigned int val)
{
    ...
    battery_log(BAT_LOG_FULL, "array_size = %d ", array_size);
    ...
    battery_log(BAT_LOG_CRTI, "NO register value match. val=%d", val);
...
static unsigned int charging_get_battery_status(void *data)
{
    ...
    *(kal_bool *) (data) = 0;    /* battery exist */
    ...
    battery_log(BAT_LOG_FULL, "[charging_get_battery_status] BATON_TDET_EN = %d", val);
    if (val) {
...
static unsigned int charging_get_platform_boot_mode(void *data)
{
    ...
    *(unsigned int *) (data) = get_boot_mode();
    ...
    battery_log(BAT_LOG_CRTI, "get_boot_mode=%d", get_boot_mode());
#endif
    return status;
}
...
void set_vusb_auxadc_irq(bool enable, bool flag)
{
    ...
    battery_log(BAT_LOG_FULL, " [%s] enable: %d, flag: %d!", __func__, enable, flag);
}
...
static void diso_polling_handler(struct work_struct *work)
{
    ...
    case AP_AUXADC_DISO_VDC_CHANNEL:
        ...
        battery_log(BAT_LOG_CRTI, "[DISO]VDC IRQ triggered, channel ==%d, flag ==%d", trigger_channel,
             trigger_flag);
        ...
        if (trigger_flag == DISO_IRQ_RISING) {
            ...
            battery_log(BAT_LOG_CRTI, " cur diso_state is %s!", DISO_state_s[2]);
        }
        ...
        if (trigger_flag == DISO_IRQ_RISING) {
            ...
            battery_log(BAT_LOG_CRTI, " cur diso_state is %s!", DISO_state_s[5]);
            ...
            DISO_data.diso_state.cur_otg_state = DISO_ONLINE;
            battery_log(BAT_LOG_CRTI, " cur diso_state is %s!", DISO_state_s[1]);
        } else
            battery_log(BAT_LOG_CRTI, "[%s] wrong trigger flag!", __func__);
#endif
        break;
    case AP_AUXADC_DISO_VUSB_CHANNEL:
        trigger_flag = DISO_Polling.vusb_polling_measure.notify_irq;
        battery_log(BAT_LOG_CRTI, "[DISO]VUSB IRQ triggered, channel ==%d, flag ==%d", trigger_channel,
             trigger_flag);
        set_vdc_auxadc_irq(DISO_IRQ_DISABLE, 0);
        set_vusb_auxadc_irq(DISO_IRQ_DISABLE, 0);
        if (trigger_flag == DISO_IRQ_FALLING) {
            ...
            DISO_data.diso_state.cur_vdc_state = DISO_ONLINE;
            DISO_data.diso_state.cur_otg_state = DISO_OFFLINE;
            battery_log(BAT_LOG_CRTI, " cur diso_state is %s!", DISO_state_s[4]);
            ...
            DISO_data.diso_state.cur_vdc_state = DISO_ONLINE;
            DISO_data.diso_state.cur_otg_state = DISO_OFFLINE;
            battery_log(BAT_LOG_CRTI, " cur diso_state is %s!", DISO_state_s[6]);
        } else
            battery_log(BAT_LOG_CRTI, "[%s] wrong trigger flag!", __func__);
    ...
    battery_log(BAT_LOG_CRTI, "[DISO]g_diso_state: 0x%x", g_diso_state);
...
static void _get_diso_interrupt_state(void)
{
    ...
    battery_log(BAT_LOG_CRTI, "[DISO] Current DC voltage mV = %d", vol);
    ...
    battery_log(BAT_LOG_CRTI, "[DISO] Current VBUS voltage mV = %d", vol);
    ...
    battery_log(BAT_LOG_CRTI, "[DISO] DISO_STATE==0x%x", diso_state);
...
static unsigned int charging_diso_init(void *data)
{
    ...
    battery_log(BAT_LOG_CRTI, "[%s] done", __func__);
...
static unsigned int charging_get_diso_state(void *data)
{
    ...
    battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task] current diso state is %s!", DISO_state_s[diso_state]);
...
static unsigned int (*const charging_func[24]) (void *data) = {
    charging_hw_init,

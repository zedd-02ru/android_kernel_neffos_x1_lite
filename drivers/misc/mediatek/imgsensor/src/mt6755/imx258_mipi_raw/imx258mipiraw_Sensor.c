...
static kal_uint8 IMX258MIPI_AWB_Data[16];
...
extern bool read_imx258_eeprom_AWB(kal_uint16 addr, BYTE* data, kal_uint32 size);
extern int read_imx258_eeprom_MID(void);
...
static void load_imx258_AWB_Data(void)
{
	kal_uint16 rg, bg, r_ratio, b_ratio, golden_rg, golden_bg;
	kal_uint16 R_GAIN, B_GAIN, Gr_GAIN, Gb_GAIN, G_GAIN;
	if (!read_imx258_eeprom_AWB(0x002C,IMX258MIPI_AWB_Data,12))
	{
		LOG_ERR("load awb data failed\n");
		return;
	}

	rg = (IMX258MIPI_AWB_Data[1] << 8) + IMX258MIPI_AWB_Data[0];
	bg = (IMX258MIPI_AWB_Data[3] << 8) + IMX258MIPI_AWB_Data[2];
	golden_rg = (IMX258MIPI_AWB_Data[7] << 8) + IMX258MIPI_AWB_Data[6];
	golden_bg = (IMX258MIPI_AWB_Data[9] << 8) + IMX258MIPI_AWB_Data[8];
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 if(read_cmos_sensor(0x0A01) == 0x01)
			 {
				 ...
				 LOG_INF("imx258 type = 0x%x(0x00=HDR,0x10=binning,0x30=No PDAF,0x40=HDD)", imx258_type);
...
static void imx258_ImageQuality_Setting(void)
{
	...
	else if (imx258_type == IMX258_HDD_TYPE)	// Based on setting ver0.30
	{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				mid = read_imx258_eeprom_MID();
				if (mid != IMX258_OFILM_MID) {
					*sensor_id = 0xFFFFFFFF;
					LOG_ERR("mid is wrong, expect is 0x%x, but read is 0x%x\n", IMX258_OFILM_MID, mid);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				back_camera_module_current = MODULE_IMX258_OFILM;
#ifdef SLT_DEVINFO_CMM
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_VC_INFO:
            ...
            case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            default:
				if(imx258_type == IMX258_HDR_TYPE || imx258_type == IMX258_HDD_TYPE) 

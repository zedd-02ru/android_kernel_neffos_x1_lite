...
{
    int  i4RetValue = 0;
	u16  i2cId_old; // [linyimin] Add for camera cal create new i2c client.
    if (gI2CBusNum == SUPPORT_I2C_BUS_NUM1) {
	i2cId_old = g_pstI2Cclient->addr;
    spin_lock(&kdsensor_drv_lock);
    g_pstI2Cclient->addr = (i2cId >> 1);
    g_pstI2Cclient->ext_flag = (g_pstI2Cclient->ext_flag)&(~I2C_DMA_FLAG);
...
u32 kdGetI2CBusNum(void) {
    u32 i2cBusNum;
    spin_lock(&kdsensor_drv_lock);
    i2cBusNum = gI2CBusNum;
    spin_unlock(&kdsensor_drv_lock);
...
struct camera_module support_camera_modules[] = {
	...
	[MODULE_OV5675_SUNWIN]     = {"ov5675", "Sunwingroup", "1.0.0"},
	[MODULE_OV5675_OFILM]      = {"ov5675", "o-film",      "1.0.0"},
	[MODULE_IMX258_OFILM]      = {"imx258", "o-film",      "1.0.0"},
	[MODULE_IMX258_TRULY]      = {"imx258", "Truly",       "1.0.0"},
	[MODULE_OV13855_SUNWIN]    = {"ov13855", "Sunwingroup","1.0.0"}, // [linyimin] add ov13855
...
/* called when user space read the back_camera_board_info interface file */
...
/* FIX-ME: linux-3.10 procfs API changed */
    ...
    if (sysfs_create_file(kernel_kobj, &dev_attr_back_camera.attr)) {
        PK_ERR("[camerahw2_probe] sysfs_create_file back camera fail\n");
        return -ENODEV;
    }

    if (sysfs_create_file(kernel_kobj, &dev_attr_front_camera.attr)) {
        PK_ERR("[camerahw2_probe] sysfs_create_file front camera fail\n");
        return -ENODEV;
    }
    /* [huangxiancong end] distinguish the camera module */

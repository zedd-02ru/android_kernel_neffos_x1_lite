...
#include "ov5675swmipi_Sensor.h"

#define PFX "ov5675sw_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV5675SWMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int RG_Ratio_Typical = 0x158; //0x15E
static int BG_Ratio_Typical = 0x183; //0x17F
...
static int read_otp(struct otp_struct *otp_ptr)
{
	...
	} else if ((otp_flag & 0x0c) == 0x04) { // base address of info group 3
		addr = 0x701b;
	}
	...
	LOG_INF("[OV5675 SUNWIN] otp_flag = 0x%x, addr = 0x%x", otp_flag, addr);
	LOG_INF("[otp_ptr] flag = 0x%x, module_id = 0x%x, lens_id = 0x%x\n",
			(*otp_ptr).flag, (*otp_ptr).module_integrator_id,(*otp_ptr).lens_id);
	LOG_INF("[otp_ptr] production: year = %4d, month = %02d, day = %2d\n",
	...
	LOG_INF("[otp_ptr] Current Unit : rg_ratio = 0x%x, bg_ratio = 0x%x \n",
		(*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);

	LOG_INF("[otp_ptr] Golden Unit : RG_Ratio_Typical = 0x%x, BG_Ratio_Typical = 0x%x \n",
...
{
	LOG_INF("OV5675SW_Sensor_Init_2lane E\n");
	//OV5675SWR1A_AM01d
...
/* [linyimin start] Modify freq for wifi */
#ifdef CONFIG_TPLINK_PRODUCT_TP904
write_cmos_sensor(0x0302, 0x9f);
#else
write_cmos_sensor(0x0302, 0x96);
#endif
/* [linyimin end] Modify freq for wifi */
...
/* [linyimin start] Modify mipi freq for wifi */
#ifdef CONFIG_TPLINK_PRODUCT_TP904
write_cmos_sensor(0x4837, 0x11);
#else
write_cmos_sensor(0x4837, 0x0d);
#endif
/* [linyimin end] Modify mipi freq for wifi */
...
{
	LOG_INF(" OV5675SWPreviewSetting_2lane enter\n");
...
{
	LOG_INF("OV5675SWCaptureSetting_2lane enter! currefps:%d\n",currefps);
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (kdGetI2CBusNum() != 2) {
		pr_err("the sub sensor should use I2C 2\n");
		...
		do {
			*sensor_id = (((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 1);
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				front_camera_module_current = MODULE_OV5675_SUNWIN;
#endif
				return ERROR_NONE;
			}
...
UINT32 OV5675SW_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	OV5675SW_MIPI_RAW_SensorInit	*/

...
PowerUp PowerOnList = {
	{
	 ...
	 {SENSOR_DRVNAME_OV5648_MIPI_RAW,
      ...
      {SENSOR_DRVNAME_OV5675_MIPI_RAW,
       ...
       {AVDD, Vol_2800, 1},
       {DVDD, Vol_1200, 1},
       {PDN, Vol_Low, 0},/* [linyimin] Modify power off sequence */
       {PDN, Vol_High, 10},
       {RST, Vol_High, 0},
       {VDD_None, Vol_2800, 5},
       },
     },
     {SENSOR_DRVNAME_OV5675SW_MIPI_RAW,
       {
       {PDN, Vol_Low, 1},
       {RST, Vol_Low, 1},
       {SensorMCLK, Vol_High, 0},
     ...
     {SENSOR_DRVNAME_OV13856_MIPI_RAW,
       {
       {PDN, Vol_Low, 1},
       {RST, Vol_Low, 1},
       {SensorMCLK, Vol_High, 0},
	 ...
	 {SENSOR_DRVNAME_OV8858_MIPI_RAW,
	  ...
	  {SENSOR_DRVNAME_IMX258XL_MIPI_RAW,
       {
       {SensorMCLK, Vol_High, 0},
       {PDN, Vol_Low, 0},
       {RST, Vol_Low, 0},
...
#define CAMERA_PIN_AVDD0    GPIO20 | 0x80000000
#define CAMERA_PIN_AVDD1    GPIO19 | 0x80000000
#if 0
#define CAMERA_PIN_AF0      GPIO51 | 0x80000000
#define CAMERA_PIN_AF1      GPIO52 | 0x80000000
#endif
...
int cam_avdd_pin[MAX_CAM_NUM] = {GPIO20 | 0x80000000, GPIO19 | 0x80000000};
//int cam_af_pin[MAX_CAM_NUM] = {GPIO51 | 0x80000000,GPIO52 | 0x80000000};
...
#if 0
BOOL cam_af_pin_enable(int camIdx, int status)
{

	if (camIdx >= MAX_CAM_NUM)
		return TRUE;

	if (mt_set_gpio_mode(cam_af_pin[camIdx],GPIO_MODE_00)) {
		PK_DBG("[CAMERA SENSOR] set gpio mode failed!! (AF%d)\n", camIdx);
		return FALSE;
	}

	if (mt_set_gpio_dir(cam_af_pin[camIdx], GPIO_DIR_OUT)) {
		PK_DBG("[CAMERA SENSOR] set gpio dir failed!! (AF%d)\n", camIdx);
		return FALSE;
	}

	if (mt_set_gpio_out(cam_af_pin[camIdx], status)) {
		PK_DBG("[CAMERA SENSOR] set gpio failed!! (AF%d)\n", camIdx);
		return FALSE;
	}
...
BOOL hwpoweron(PowerInformation pwInfo, char *mode_name, int camIdx)
{
	if (pwInfo.PowerType == AVDD) {
		if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (TRUE != cam_avdd_pin_enable(camIdx, GPIO_OUT_ONE)) {
...
#if 0
			if (TRUE != cam_af_pin_enable(camIdx, GPIO_OUT_ONE)) {
				PK_ERR("[CAMERA SENSOR] Fail to power up AVDD_EN\n");
				return FALSE;
			}
#endif
...
BOOL hwpowerdown(PowerInformation pwInfo, char *mode_name, int camIdx)
{
	if (pwInfo.PowerType == AVDD) {
		if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			...
			if (TRUE != cam_avdd_pin_enable(camIdx, GPIO_OUT_ZERO)) {
...
#if 0
			if (TRUE != cam_af_pin_enable(camIdx, GPIO_OUT_ZERO)) {
				PK_ERR("[CAMERA SENSOR] Fail to power down AF_EN\n");
				return FALSE;
			}
#endif
		} else {
	...
	if(has_mipi_switch){
		...
		for (pwListIdx = 0; pwListIdx < 16; pwListIdx++) {
			if (currSensorName && (PowerOnList.PowerSeq[pwListIdx].SensorName != NULL)
				...
				if ((0 == strcmp(SENSOR_DRVNAME_OV13855_MIPI_RAW, currSensorName)) \
					|| (0 == strcmp(SENSOR_DRVNAME_OV13856_MIPI_RAW, currSensorName)) \
					|| (0 == strcmp(SENSOR_DRVNAME_IMX258_MIPI_RAW, currSensorName)) \
					|| (0 == strcmp(SENSOR_DRVNAME_IMX258XL_MIPI_RAW, currSensorName)) ) {
					camIdx = 0;
				} else if ((0 == strcmp(SENSOR_DRVNAME_OV5675_MIPI_RAW, currSensorName)) \
							|| (0 == strcmp(SENSOR_DRVNAME_OV5675SW_MIPI_RAW, currSensorName)) \
					...
					if (PowerOnList.PowerSeq[pwListIdx].PowerInfo[pwIdx].
						...
						if (hwpoweron
						    (PowerOnList.PowerSeq[pwListIdx].
						     PowerInfo[pwIdx], mode_name, camIdx) == FALSE)

...
#define OV13855_EEPROM_WRITE_ID   0xA2//eeprom id 0xA0
...
BYTE ov13855_eeprom_awb_data[32] = {0};/* [linyimin] Add awb cali */
...
static bool selective_read_eeprom(kal_uint16 addr, BYTE* data)
{
    ...
    if(addr > OV13855_MAX_OFFSET)
        return false;
	if(iReadRegI2C(pu_send_cmd, 2, (u8*)data, 1, OV13855_EEPROM_READ_ID)<0) // [linyimin] Fix read id err
		return false;
    return true;
}
...
bool read_ov13855_eeprom_AWB(kal_uint16 addr, BYTE* data, kal_uint32 size){
	BYTE flag;
	...
	if (!awb_get_done) {
		...
		if(!_read_ov13855_eeprom(0x0021, OV13855_eeprom_data, 21)){
			pr_err("Read awb error\n");
			return false;
		}

		for(i=0; i<20; i++) {
			sum += (int)(OV13855_eeprom_data[i]);
		}

		checksum = OV13855_eeprom_data[20];
		...
		memcpy(ov13855_eeprom_awb_data, OV13855_eeprom_data, 20);
		awb_get_done = 1;
	}
	memcpy(data, ov13855_eeprom_awb_data, 20);
	return true;
}
/* [linyimin end] */

/* [linyimin start] Read 13855 mid */
int read_imx13855_eeprom_MID(void)
{
	BYTE mid;

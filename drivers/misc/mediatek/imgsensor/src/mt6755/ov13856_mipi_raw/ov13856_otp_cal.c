...
#include "ov13856mipiraw_Sensor.h"
#include "ov13856_otp.h"

#if 1
extern int iReadReg(u16 a_u2Addr , u8 * a_puBuff , u16 i2cId);
extern int iWriteReg(u16 a_u2Addr , u32 a_u4Data , u32 a_u4Bytes , u16 i2cId);

//#define OV13856_R2A_write_i2c(addr, para) iWriteReg((u16) addr , (u32) para , 1, OV13856MIPI_WRITE_ID)
#define PFX "OV13856_R2A_OTP"
//#define LOG_INF(format, args...)	//xlog_printk(ANDROID_LOG_INFO   , PFX, "[%s] " format, __FUNCTION__, ##args)
#define LOG_INF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)

#define Delay(ms)  mdelay(ms)
static unsigned char OV13856MIPI_WRITE_ID = 0X00;

kal_uint16 OV13856_R2A_read_i2c(kal_uint32 addr)
{
    kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,OV13856MIPI_WRITE_ID);
    return get_byte;
}

kal_uint16 OV13856_R2A_write_i2c(kal_uint32 addr, kal_uint32 para)
{
		iWriteReg((u16) addr , (u32) para , 1, OV13856MIPI_WRITE_ID);
		return 1;
}
#endif
...
void otp_cali(unsigned char writeid)
{
	...
	LOG_INF("otp_cali start \n");
	OV13856MIPI_WRITE_ID = writeid;
	LOG_INF("OV13856MIPI_WRITE_ID: 0x%x \n", OV13856MIPI_WRITE_ID);
	memset(&current_otp, 0, sizeof(struct otp_struct));

 // otp_TestValue = OV13856_R2A_read_i2c(0x3501);
 // LOG_INF("otp_TestValue01: %x \n", otp_TestValue);
 // OV13856_R2A_write_i2c(0x3501, 0x78);
//	Delay(10);
  // otp_TestValue = OV13856_R2A_read_i2c(0x3501);
  //LOG_INF("otp_TestValue02: %x \n", otp_TestValue);

	read_otp(&current_otp);
	apply_otp(&current_otp);
	LOG_INF("otp_cali end \n");
//	printk(" OTP OK chaoang01 \n");
}

int Decode_13856R2A(unsigned char*pInBuf, unsigned char* pOutBuf)
{
	if(pInBuf != NULL)
	{
...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	//set 0x5002[1] to "0"
	int temp1=0;
	temp1 = OV13856_R2A_read_i2c(0x5000);
	LOG_INF("OV13856_R2A_read_i2c(0x5000) 0x%x \n", temp1);
	OV13856_R2A_write_i2c(0x5000, (0x00 & 0x10) | (temp1 & (~0x10)));
	Delay(20);
	// read OTP into buffer
	OV13856_R2A_write_i2c(0x3d84, 0xC0);
	OV13856_R2A_write_i2c(0x3d88, 0x72); // OTP start address
	OV13856_R2A_write_i2c(0x3d89, 0x20);
	OV13856_R2A_write_i2c(0x3d8A, 0x73); // OTP end address
	OV13856_R2A_write_i2c(0x3d8B, 0xBE);
	OV13856_R2A_write_i2c(0x3d81, 0x01); // load otp into buffer
	Delay(50);
//	otp_flag = 0xff;
	// OTP base information and WB calibration data
	otp_flag = OV13856_R2A_read_i2c(0x7220);
	...
	if(addr != 0) {
		...
		(*otp_ptr).module_integrator_id = OV13856_R2A_read_i2c(addr);
		(*otp_ptr).lens_id = OV13856_R2A_read_i2c( addr + 1);
		(*otp_ptr).production_year = OV13856_R2A_read_i2c( addr + 2);
		(*otp_ptr).production_month = OV13856_R2A_read_i2c( addr + 3);
		(*otp_ptr).production_day = OV13856_R2A_read_i2c(addr + 4);
		temp = OV13856_R2A_read_i2c(addr + 7);
		(*otp_ptr).rg_ratio = (OV13856_R2A_read_i2c(addr + 5)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (OV13856_R2A_read_i2c(addr + 6)<<2) + ((temp>>4) & 0x03);
	}
	else {
	...
	// OTP VCM Calibration
#if 0
	otp_flag = OV13856_R2A_read_i2c(0x73ac);
	...
	if(addr != 0) {
		(*otp_ptr).flag |= 0x20;
		temp = OV13856_R2A_read_i2c(addr + 2);
		(* otp_ptr).VCM_start = (OV13856_R2A_read_i2c(addr)<<2) | ((temp>>6) & 0x03);
		(* otp_ptr).VCM_end = (OV13856_R2A_read_i2c(addr + 1) << 2) | ((temp>>4) & 0x03);
  ...
  //otp_flag = 0xff;
	...
	otp_flag = OV13856_R2A_read_i2c(0x7231);
...
if(addr != 0) {
		for(i=0;i<186;i++) {
			(* otp_ptr).lenc[i]= OV13856_R2A_read_i2c(addr + i);
		...
		if(Decode_13856R2A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<360;i++)
			{
		...
		(* otp_ptr).checksumLSC=OV13856_R2A_read_i2c(addr + 186);
		(* otp_ptr).checksumOTP=OV13856_R2A_read_i2c(addr + 187);
		(* otp_ptr).checksumTotal=OV13856_R2A_read_i2c(addr + 188);
	...
	{
		OV13856_R2A_write_i2c(i,0); // clear OTP buffer, recommended use continuous write to accelarate
	}

	//set 0x5002[1] to "1"
	temp1 = OV13856_R2A_read_i2c(0x5000);
	OV13856_R2A_write_i2c(0x5000, (0x10 & 0x10) | (temp1 & (~0x10)));
...
int apply_otp(struct otp_struct *otp_ptr)
{
	...
	int i=0;
	unsigned char lenc_out[360];
	LOG_INF(" OTP ReadState OTPINFO AWB VCM LSC XXXX0000\n");
	LOG_INF(" apply_otp (*otp_ptr).flag : %x \n", (*otp_ptr).flag);
	LOG_INF(" RG_Ratio_Typical: %d \n", RG_Ratio_Typical);
	LOG_INF(" BG_Ratio_Typical: %d \n", BG_Ratio_Typical);
	...
	LOG_INF("Apply R_gain=0x%x\n", R_gain);
	LOG_INF("Apply B_gain=0x%x\n", B_gain);
	LOG_INF("Apply G_gain=0x%x\n", G_gain);
		// update sensor WB gain
		if (R_gain>0x400) {
			OV13856_R2A_write_i2c(0x5104, R_gain>>8);
			OV13856_R2A_write_i2c(0x5105, R_gain & 0x00ff);
		}

		if (G_gain>0x400) {
			OV13856_R2A_write_i2c(0x5102, G_gain>>8);
			OV13856_R2A_write_i2c(0x5103, G_gain & 0x00ff);
		}

		if (B_gain>0x400) {
			OV13856_R2A_write_i2c(0x5100, B_gain>>8);
			OV13856_R2A_write_i2c(0x5101, B_gain & 0x00ff);
		}
	}
	...
	if ((*otp_ptr).flag & 0x10) {
		...
		temp = OV13856_R2A_read_i2c(0x5000);
		temp = 0x04 | temp;
		OV13856_R2A_write_i2c(0x5000, temp);
		...
		/***For function Decode_13856R2A(unsigned char*pInBuf, unsigned char* pOutBuf),please refer to lc42.h***/
		if(Decode_13856R2A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<360 ;i++) {
				LOG_INF(" apply OTP lenc_out[%d]:%x \n", i, lenc_out[i]);
				OV13856_R2A_write_i2c(0x5500 + i, lenc_out[i]);
			}
		}
	}
	return 0;

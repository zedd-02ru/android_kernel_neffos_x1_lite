 ...
 * Setting version:
 * ------------
 *   update full pd setting for OV13856EB_03B
 *------------------------------------------------------------------------------
...
#define PFX "OV13856_camera_sensor"
#define LOG_INF(fmt, args...)   pr_debug(PFX "[%s] " fmt, __FUNCTION__, ##args)
#define LOG_ERR(fmt, args...)   pr_err(PFX "[%s] " fmt, __FUNCTION__, ##args)

extern bool read_ov13856_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern void kdSetI2CSpeed(u16 i2cSpeed);

extern int read_imx13856_eeprom_MID(void);

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static kal_uint8 OV13856MIPI_AWB_Data[20];/* [linyimin end] Add awb cali */

static imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV13856_SENSOR_ID,
...
#if 1 // [linyimin] Modify hs_video win size
	.hs_video = {
		.pclk = 108250560,
		.linelength = 1122,
		.framelength = 810,
		...
		.max_framerate = 1200,// [linyimin] Modify hs_video win size
	},
#else
	.hs_video = {
		.pclk = 108250560,
...
/* [linyimin start] Modify mipi freq for ratio */
...
// [linyimin] Modify hs_video win size
#if 1//1024x768
...
static kal_uint32 return_sensor_id(void)
{
    ...
    return ( (read_cmos_sensor(0x300a) << 16) | (read_cmos_sensor(0x300b) << 8) | read_cmos_sensor(0x300c)) + 1;
#else
    return 0xFFFFFFFF;
#endif
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                ...
                mid = read_imx13856_eeprom_MID();
                if (mid != OV13855_SUNWIN_MID) {
                    *sensor_id = 0xFFFFFFFF;
                    LOG_ERR("mid is wrong, expect is 0x%x, but read is 0x%x\n", OV13855_SUNWIN_MID, mid);
                    return ERROR_SENSOR_CONNECT_FAIL;
                }
                back_camera_module_current = MODULE_OV13855_SUNWIN;
                /* [linyimin end] */
...
extern bool read_ov13856_eeprom_AWB(kal_uint16 addr, BYTE* data, kal_uint32 size);
#define GAIN_DEFAULT 0x0100
#define GOLDEN_RG_VALUE 0x147 // dev #13
#define GOLDEN_BG_VALUE 0x18d // dev #13
void ov_13856_apply_awb_otp(void)
{
	int rg=0;
	int bg=0;
	int golden_rg, golden_bg;
	int R_gain, B_gain, G_gain, Base_gain;
	LOG_INF(" OTP ReadState OTPINFO AWB VCM LSC XXXX0000\n");

	if (!read_ov13856_eeprom_AWB(0x0020,OV13856MIPI_AWB_Data,12))
	{
		LOG_ERR("read awb data failed\n");
		return;
	}
	// apply OTP WB Calibration

	rg = (OV13856MIPI_AWB_Data[0] << 8) + OV13856MIPI_AWB_Data[1];
	bg = (OV13856MIPI_AWB_Data[2] << 8) + OV13856MIPI_AWB_Data[3];
	golden_rg = GOLDEN_RG_VALUE;
	golden_bg = GOLDEN_BG_VALUE;
...
static kal_uint32 open(void)
{
	...
	//apply_awb_otp();/* [linyimin] Add awb cali */
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
    ...
    LOG_ERR("scenario_id = %d, framerate = %d\n", scenario_id, framerate);
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_ov13856_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
...
UINT32 OV13856_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{

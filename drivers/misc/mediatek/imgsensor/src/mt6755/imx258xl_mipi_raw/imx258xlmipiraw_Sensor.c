...
#include "imx258xlmipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "IMX258XL_camera_sensor"
#define LOG_1 LOG_INF("IMX258XL,MIPI 1LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX258XL_SENSOR_ID, //IMX258XLMIPI_SENSOR_ID,  /*sensor_id = 0x2680*/ //record sensor id defined in Kd_imgsensor.h

	.checksum_value = 0x87642414, //checksum value for Camera Auto Test
...
#define IMX258XL_HDR_TYPE (0x00)
#define IMX258XL_BINNING_TYPE (0x10)
#define IMX258XL_NOPDAF_TYPE	(0x30)
#define IMX258XL_HDD_TYPE (0x40)	// IMX258XL-0AUH5
kal_uint16 imx258xl_type = 0;/*0x00=HDR type, 0x10=binning type*/
/* Sensor output window information */
/*according toIMX258XL datasheet p53 image cropping*/
...
static SET_PD_BLOCK_INFO_T imgsensor_pd_info =
{
    ...
    .i4SubBlkW =8,
    .i4SubBlkH =16,
    .i4PosL = {{25,30},{33,30},{41,30},{49,30},{29,46},{37,46},{45,46},{53,46}},
    .i4PosR = {{25,34},{33,34},{41,34},{49,34},{29,50},{37,50},{45,50},{53,50}},
};

static SENSOR_VC_INFO_STRUCT SENSOR_VC_INFO_HDD[3]=
{
	/* Preview mode setting */
  	{0x03, 0x0a,	 0x00,	 0x08, 0x40, 0x00,
   	0x00, 0x2b, 0x0834, 0x0618, 0x00, 0x35, 0x0280, 0x0001,
...
static SET_PD_BLOCK_INFO_T imgsensor_pd_info_HDD =
{
    .i4OffsetX = 24,
    .i4OffsetY = 24,
    .i4PitchX = 32,
...
#define IMX258XLMIPI_MaxGainIndex (154)
kal_uint16 IMX258XLMIPI_sensorGainMapping[IMX258XLMIPI_MaxGainIndex][2] ={
	{64 ,0	},
	{65 ,8	},
	{66 ,16 },
	{67 ,25 },
...
static kal_uint8 IMX258XLMIPI_SPC_Data[126];
static kal_uint8 IMX258XLMIPI_AWB_Data[16];
...
extern u32 kdGetI2CBusNum(void);
...
extern bool read_imx258xl_eeprom( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx258xl_eeprom_SPC( kal_uint16 addr, BYTE* data, kal_uint32 size);
extern bool read_imx258xl_eeprom_AWB(kal_uint16 addr, BYTE* data, kal_uint32 size);
extern int read_imx258xl_eeprom_MID(void);

static void load_imx258xl_SPC_Data(void)
{
	kal_uint16 i;
	if(SPC_data_done == false)
	{
		if(!read_imx258xl_eeprom_SPC(0x0F73,IMX258XLMIPI_SPC_Data,126))
		{
			LOG_ERR("load imx258xl failed\n");
			return;
		}
		SPC_data_done = true;
	}
	...
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD04C+i, IMX258XLMIPI_SPC_Data[i]);
		//LOG_INF("SPC_Data[%d] = %d\n", i, IMX258XLMIPI_SPC_Data[i]);
		//LOG_INF("read_SPC_Data[%d] = %d\n", i, read_cmos_sensor(0xD04C+i));
	}
	for(i=0; i<63; i++)
	{
		write_cmos_sensor(0xD08C+i, IMX258XLMIPI_SPC_Data[i+63]);
		//LOG_INF("SPC_Data[%d] = %d\n", i+63, IMX258XLMIPI_SPC_Data[i+63]);
...
static void load_imx258xl_AWB_Data(void)
{
	kal_uint16 rg, bg, r_ratio, b_ratio, golden_rg, golden_bg;
	kal_uint16 R_GAIN, B_GAIN, Gr_GAIN, Gb_GAIN, G_GAIN;
	return;
	if (!read_imx258xl_eeprom_AWB(0x002C,IMX258XLMIPI_AWB_Data,12))
	{
		LOG_ERR("load awb data failed\n");
		return;
	}

	rg = (IMX258XLMIPI_AWB_Data[1] << 8) + IMX258XLMIPI_AWB_Data[0];
	bg = (IMX258XLMIPI_AWB_Data[3] << 8) + IMX258XLMIPI_AWB_Data[2];
	golden_rg = (IMX258XLMIPI_AWB_Data[7] << 8) + IMX258XLMIPI_AWB_Data[6];
	golden_bg = (IMX258XLMIPI_AWB_Data[9] << 8) + IMX258XLMIPI_AWB_Data[8];

	r_ratio = 1024 * golden_rg / rg;
	b_ratio = 1024 * golden_bg / bg;

	if (!r_ratio || !b_ratio) {
		pr_err("get awb error\n");
		return;
	}

	if (r_ratio >= 512) {
		if (b_ratio >= 512) {
			R_GAIN = (kal_uint16)(GAIN_DEFAULT * r_ratio / 1024);
			G_GAIN = GAIN_DEFAULT;
			B_GAIN = (kal_uint16)(GAIN_DEFAULT * b_ratio / 1024);
		} else {
			R_GAIN = (kal_uint16)(GAIN_DEFAULT * r_ratio / b_ratio);
			G_GAIN = (kal_uint16)(GAIN_DEFAULT * 1024 / b_ratio);
			B_GAIN = GAIN_DEFAULT;
		}
	} else {
		if (b_ratio >= 1024) {
			R_GAIN = GAIN_DEFAULT;
			G_GAIN = (kal_uint16)(GAIN_DEFAULT * 1024 / r_ratio);
			B_GAIN = (kal_uint16)(GAIN_DEFAULT * b_ratio / r_ratio);
		} else {
			Gr_GAIN = (kal_uint16)(GAIN_DEFAULT * 1024 / r_ratio);
			Gb_GAIN = (kal_uint16)(GAIN_DEFAULT * 1024 / b_ratio);
...
//	kal_uint32 tmp = 0;
	...
	if(write_cmos_sensor(0x0A02, 0x0F)==0){
		...
		while(retry--)
		{
			 ...
			 {
			 	 imx258xl_type = read_cmos_sensor(0x0A2E);
				 LOG_INF("imx258xl type = 0x%x(0x00=HDR,0x10=binning,0x30=No PDAF,0x40=HDD)", imx258xl_type);
...
/* [linyimin start] Support long exposure*/
static void set_long_shutter(kal_uint32 shutter)
{
    unsigned long flags;
    kal_uint8 shift = 0;

    LOG_INF("set shutter: %d", shutter);

    spin_lock_irqsave(&imgsensor_drv_lock, flags);
    imgsensor.shutter = shutter;
    spin_unlock_irqrestore(&imgsensor_drv_lock, flags);

    while((shutter & 0xffff0000) || (shutter > (imgsensor_info.max_frame_length - imgsensor_info.margin))) {
        shutter = shutter >> 1;
        shift++;
    }

    LOG_INF("set left shift: %d", shift);
    ...
    write_cmos_sensor(0x3002, shift);

    set_shutter((kal_uint16)shutter);

}
/* [linyimin end] Support long exposure*/
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
    ...
    for (i = 0; i < IMX258XLMIPI_MaxGainIndex; i++) {
        if(gain <= IMX258XLMIPI_sensorGainMapping[i][0]){
            break;
        }
    }
    if(gain != IMX258XLMIPI_sensorGainMapping[i][0])
            LOG_ERR("Gain mapping don't correctly:%d %d \n", gain, IMX258XLMIPI_sensorGainMapping[i][0]);
    return IMX258XLMIPI_sensorGainMapping[i][1];
}
...
static void imx258xl_ImageQuality_Setting(void)
{
	if((imx258xl_type == IMX258XL_HDR_TYPE)||(imx258xl_type == IMX258XL_NOPDAF_TYPE))
	{
	...
	else if(imx258xl_type == IMX258XL_BINNING_TYPE)
	{
		write_cmos_sensor(0x94C7,0xFF);
		write_cmos_sensor(0x94C8,0xFF);
		write_cmos_sensor(0x94C9,0xFF);
	...
	else if (imx258xl_type == IMX258XL_HDD_TYPE)	// Based on setting ver0.30
	{
		write_cmos_sensor(0x94C7, 0x00);
		write_cmos_sensor(0x94C8, 0x00);
		write_cmos_sensor(0x94C9, 0x00);
		write_cmos_sensor(0x95C7, 0x00);
		write_cmos_sensor(0x95C8, 0x00);
		write_cmos_sensor(0x95C9, 0x00);
		write_cmos_sensor(0x95C4, 0x2F);
		write_cmos_sensor(0x95C5, 0x2F);
		write_cmos_sensor(0x95C6, 0x2F);
		write_cmos_sensor(0x94C1, 0x00);
		write_cmos_sensor(0x94C2, 0x00);
		write_cmos_sensor(0x94C3, 0x00);
		write_cmos_sensor(0x95C1, 0x00);
		write_cmos_sensor(0x95C2, 0x00);
		write_cmos_sensor(0x95C3, 0x00);
		write_cmos_sensor(0x95BE, 0x0F);
		write_cmos_sensor(0x95BF, 0x0F);
		write_cmos_sensor(0x95C0, 0x0F);
		write_cmos_sensor(0x94D0, 0x74);
		write_cmos_sensor(0x94D1, 0x74);
		write_cmos_sensor(0x94D2, 0x74);
		write_cmos_sensor(0x95D0, 0x74);
		write_cmos_sensor(0x95D1, 0x74);
		write_cmos_sensor(0x95D2, 0x74);
		write_cmos_sensor(0x95CD, 0x30);
		write_cmos_sensor(0x95CE, 0x30);
		write_cmos_sensor(0x95CF, 0x30);
		write_cmos_sensor(0x95CA, 0x40);
		write_cmos_sensor(0x95CB, 0x40);
		write_cmos_sensor(0x95CC, 0x40);
		write_cmos_sensor(0x900E, 0x32);
		write_cmos_sensor(0x94E2, 0x80);
		write_cmos_sensor(0x94E3, 0x80);
		write_cmos_sensor(0x94E4, 0x80);
		write_cmos_sensor(0x95E2, 0x80);
		write_cmos_sensor(0x95E3, 0x80);
		write_cmos_sensor(0x95E4, 0x80);
		write_cmos_sensor(0x94DF, 0x6E);
		write_cmos_sensor(0x94E0, 0x6E);
		write_cmos_sensor(0x94E1, 0x6E);
		write_cmos_sensor(0x95DF, 0x2D);
		write_cmos_sensor(0x95E0, 0x2D);
		write_cmos_sensor(0x95E1, 0x2D);
		write_cmos_sensor(0x7FCC, 0x01);
		write_cmos_sensor(0x7B78, 0x00);
		write_cmos_sensor(0x9401, 0x35);
		write_cmos_sensor(0x9403, 0x23);
		...
		write_cmos_sensor(0xD00A, 0x01);
		write_cmos_sensor(0xD00B, 0x02);
		write_cmos_sensor(0xD00D, 0x02);
		write_cmos_sensor(0xD00E, 0x05);
		write_cmos_sensor(0xD00F, 0x01);
		write_cmos_sensor(0xD011, 0x02);
		write_cmos_sensor(0xD012, 0x09);
		write_cmos_sensor(0xD013, 0x02);
		write_cmos_sensor(0xD015, 0x0A);
		write_cmos_sensor(0xD016, 0x05);
		write_cmos_sensor(0xD017, 0x01);
		write_cmos_sensor(0xD019, 0x0A);
		write_cmos_sensor(0xD01A, 0x09);
		write_cmos_sensor(0xD01B, 0x02);
		write_cmos_sensor(0xD01D, 0x06);
		write_cmos_sensor(0xD01E, 0x15);
		write_cmos_sensor(0xD01F, 0x01);
		write_cmos_sensor(0xD021, 0x06);
		write_cmos_sensor(0xD022, 0x19);
		write_cmos_sensor(0xD023, 0x02);
		write_cmos_sensor(0xD025, 0x0E);
		write_cmos_sensor(0xD026, 0x15);
		write_cmos_sensor(0xD027, 0x01);
		write_cmos_sensor(0xD029, 0x0E);
		write_cmos_sensor(0xD02A, 0x19);
		write_cmos_sensor(0xD02B, 0x02);
	}
}
...
static void sensor_init(void)
{
	...
	imx258xl_ImageQuality_Setting();
	if(imx258xl_type != IMX258XL_NOPDAF_TYPE)
	{
		load_imx258xl_SPC_Data();
...
static void preview_setting(void)
{
	...
	if(imx258xl_type != IMX258XL_HDD_TYPE)
	{
	...
	if(imx258xl_type == IMX258XL_BINNING_TYPE)
	{
	...
	if(imx258xl_type == IMX258XL_HDD_TYPE)
	{
		...
		write_cmos_sensor(0x94BE,0x1F);
		write_cmos_sensor(0x94BF,0x1F);
		write_cmos_sensor(0x94C0,0x1F);
		write_cmos_sensor(0x94C4,0x2F);
		write_cmos_sensor(0x94C5,0x2F);
		write_cmos_sensor(0x94C6,0x2F);
		write_cmos_sensor(0x94CA,0x50);
		write_cmos_sensor(0x94CB,0x50);
		write_cmos_sensor(0x94CC,0x50);
		write_cmos_sensor(0x94CD,0x40);
		write_cmos_sensor(0x94CE,0x40);
		write_cmos_sensor(0x94CF,0x40);
	}
...
static void hs_video_setting(void)
{
	...
	if(imx258xl_type == IMX258XL_HDR_TYPE)
	{
		write_cmos_sensor(0x7BCD,0x00);
	}
	else if(imx258xl_type == IMX258XL_BINNING_TYPE)
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	s_DEVINFO_ccm->device_ic = "IMX258XL";
	s_DEVINFO_ccm->device_version = "HI";
	s_DEVINFO_ccm->device_info = "200W";
#endif

	if (kdGetI2CBusNum() != 0) {
		LOG_ERR("the main sensor should use I2C num 0\n");
		...
		do {
			*sensor_id = return_sensor_id() + 1;
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				mid = read_imx258xl_eeprom_MID();
				if (mid != MODULE_TRULY_MID) {
					*sensor_id = 0xFFFFFFFF;
					LOG_ERR("mid is wrong, expect is 0x%x, but read is 0x%x\n", MODULE_TRULY_MID, mid);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				back_camera_module_current = MODULE_IMX258_TRULY;
#ifdef SLT_DEVINFO_CMM
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	else if(imx258xl_type == IMX258XL_HDD_TYPE)
...
//    unsigned long long *feature_return_para=(unsigned long long *) feature_para;
	...
	if(!((feature_id == SENSOR_FEATURE_CHECK_SENSOR_ID) || (feature_id == SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO)))
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_SET_ESHUTTER:
			// [linyimin] Support long exposure
			set_long_shutter((UINT32)*feature_data);
			break;
		...
		case SENSOR_FEATURE_GET_PDAF_DATA:
			...
			read_imx258xl_eeprom((kal_uint16 )(*feature_data),(char*)(uintptr_t)(*(feature_data+1)),(kal_uint32)(*(feature_data+2)));
			break;
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
			...
			switch (*feature_data_32) {
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
					if((imx258xl_type == IMX258XL_HDR_TYPE)||(imx258xl_type == IMX258XL_NOPDAF_TYPE) || (imx258xl_type == IMX258XL_HDD_TYPE))
			...
			switch (*feature_data) {
				case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
					if(imx258xl_type == IMX258XL_HDR_TYPE)
						...
						memcpy((void *)PDAFinfo,(void *)&imgsensor_pd_info_HDD,sizeof(SET_PD_BLOCK_INFO_T));
					else
		...
		case SENSOR_FEATURE_GET_VC_INFO:
            ...
            case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
				if(imx258xl_type == IMX258XL_HDR_TYPE)
					...
					memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO_HDD[1],sizeof(SENSOR_VC_INFO_STRUCT));
				else
            ...
            case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
				if(imx258xl_type == IMX258XL_HDR_TYPE)
					...
					memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO_HDD[2],sizeof(SENSOR_VC_INFO_STRUCT));
				else
					memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO_Binning[2],sizeof(SENSOR_VC_INFO_STRUCT));
                break;
            case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            default:
				if(imx258xl_type == IMX258XL_HDR_TYPE || imx258xl_type == IMX258XL_HDD_TYPE) 
                	memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO[0],sizeof(SENSOR_VC_INFO_STRUCT));
        else if(imx258xl_type == IMX258XL_HDD_TYPE)
					memcpy((void *)pvcinfo,(void *)&SENSOR_VC_INFO_HDD[0],sizeof(SENSOR_VC_INFO_STRUCT));
				else
...
UINT32 IMX258XL_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
/*UINT32 IMX258XL_MIPI_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)*/

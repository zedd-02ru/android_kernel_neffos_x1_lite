...
static int RG_Ratio_Typical= 0x16C ; //364
static int BG_Ratio_Typical= 0x17D ; //381
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	if (kdGetI2CBusNum() != 2) {
		LOG_INF("the main sensor should use I2C num 2\n");
...
#ifdef OV5675OTP
				read_otp(&otp_ptr);
				if (otp_ptr.module_integrator_id != MODULE_OFILM_MID) {
					*sensor_id = 0xFFFFFFFF;
					LOG_ERR("mid is wrong, expect is 0x%x, but read is 0x%x\n", MODULE_OFILM_MID, otp_ptr.module_integrator_id);
					return ERROR_SENSOR_CONNECT_FAIL;
				}
				front_camera_module_current = MODULE_OV5675_OFILM;
#endif
				return ERROR_NONE;
			}

...
static int i2c_read(u8 a_u2Addr, u8 *a_puBuff)
{
	...
	// [linyimin] Correct ret value should be 1
	if (i4RetValue != 1) {
...
/* [linyimin start] Add dw9718 power on func */
static int DW9718AF_powerOn(void)
{
	int i4RetValue = 0;
	char PD[2] = { (char)(0x00), (char)(0x00) };

	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, PD, 2);
	if (i4RetValue < 0) {
		LOG_INF(" DW9718AF_powerOn I2C send failed!!\n");
		return -1;
	}
	return 0;
}
/* [linyimin end] Add dw9718 power on func */

/* [linyimin start] Add dw9718 power off func */
static int DW9718AF_powerOff(void)
{
	int i4RetValue = 0;
	char PD[2] = { (char)(0x00), (char)(0x01) };

	i4RetValue = i2c_master_send(g_pstAF_I2Cclient, PD, 2);
	if (i4RetValue < 0) {
		LOG_INF(" DW9718AF_powerOff I2C send failed!!\n");
		return -1;
	}
	return 0;
}
/* [linyimin end] Add dw9718 power off func */
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		DW9718AF_powerOn();// [linyimin] disable PD
...
int DW9718AF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		LOG_INF("Wait\n");
		DW9718AF_powerOff();// [linyimin] enable PD
	}
...
void DW9718AF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	...
	g_pstAF_I2Cclient->timing = 400; // [linyimin] Modify i2c freq

...
/* [lidebiao start] Set transport mode for storage */
#if defined(CONFIG_TPLINK_PRODUCT_TP902) || defined(CONFIG_TPLINK_PRODUCT_TP904)
extern void bq24296_set_batfet_disable(unsigned int val);
extern void bq24296_set_watchdog(unsigned int val);
#endif
/* [zhangdong] add tp910 support */
...
extern void bq25890_set_batfet_disable(unsigned int val);
...
/* [lidebiao start] Set transport mode for storage */
	if (is_set_transport_mode()) {
		...
		while (1) {
			...
			rtc_xinfo("wait for usb plug out!\n");
			mdelay(500);
		}
#if defined(CONFIG_TPLINK_PRODUCT_TP902) || (defined(CONFIG_TPLINK_PRODUCT_TP904))
...
/* [zhangdong] add tp910 support */
#if defined(CONFIG_TPLINK_PRODUCT_TP910)
		bq2560x_set_watchdog(0x0);
		bq2560x_set_batfet_disable(0x1);
#endif

#if defined(CONFIG_TPLINK_PRODUCT_TP903)
		bq25890_set_wd_timer(0x0);
		bq25890_set_batfet_disable(0x1);
#endif
	}
/* [lidebiao end] */
...
/* [liguanxiong start] */
static ssize_t reboot2mmi_show(struct device *dev,
						struct device_attribute *attr, char *buf)
{
	return sprintf(buf,"%d", check_rtc_mmi_mode());
}
static ssize_t reboot2mmi_store(struct device *dev, struct device_attribute *attr,
						const char *buf, size_t count)
{
	...
	printk("set  reboot2mmi %d done!\n", cmd);
	return count;
}
static DEVICE_ATTR(reboot2mmi, 0660, reboot2mmi_show, reboot2mmi_store);
/* [liguanxiong end] */
...
static int rtc_pdrv_probe(struct platform_device *pdev)
{
	...
	/* [liguanxiong] */
	ret = sysfs_create_file(kernel_kobj, &dev_attr_reboot2mmi.attr);

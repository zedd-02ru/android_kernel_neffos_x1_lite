...
char* get_pcb_version_s(void)
{
	...
	/* [TP903] EVT:0.9 DVT1:0.6V DVT2:1.2V DVT3:0.3V PVT:0V */
	if (pcb_version_voltage > 1050) {
		...
		index = DVT1;
	}else if (pcb_version_voltage > 150) {
		index = DVT3;
	} else {
		index = PVT;
	}
#elif defined (CONFIG_TPLINK_PRODUCT_TP902)
	/* [TP902] EVT:0.9 DVT1:0.6V DVT3:1.2V PVT:0V MP:0.3V */
	if (pcb_version_voltage > 1050) {
		index = DVT3;
	}else if (pcb_version_voltage > 750) {
		index = EVT;
...
#else
	/* [TP904] EVT:0.9V DVT1:0.6V DVT2:1.2V DVT3:0.3V PVT:0V */
	if (pcb_version_voltage > 1050) {
		index = DVT2;
	}else if (pcb_version_voltage > 750) {
		index = EVT;

	...
	for (i = 0; i < MAX_M_V_SENSOR; i++) {
		...
		if (ID_M_V_MAGNETIC == i) {
			...
			if (true ==  cxt->is_first_data_after_enable_mag) {
				m_pre_ns = cur_ns;
				cxt->is_first_data_after_enable_mag = false;  /* [yanlin] Edit for enable error */
				/* filter -1 value */
			...
			/* [yanlin start] Dont need to report */
			while ((cur_ns - m_pre_ns) >= delay_ms*1800000LL) {
				MAG_ERR("--- m_pre_ns(%d) behind cur_ns(%d) exceed delay_ms(%d) ---\n",
						(int)m_pre_ns, (int)cur_ns, (int)delay_ms);
		...
		if (ID_M_V_ORIENTATION == i) {
			...
			if (true ==  cxt->is_first_data_after_enable_ori) {
				o_pre_ns = cur_ns;
				cxt->is_first_data_after_enable_ori = false;  /* [yanlin] Edit for enable error */
				/* filter -1 value */
				if (MAG_INVALID_VALUE == cxt->drv_data[i].mag_data.values[0] ||
				MAG_INVALID_VALUE == cxt->drv_data[i].mag_data.values[1] ||
				MAG_INVALID_VALUE == cxt->drv_data[i].mag_data.values[2]) {
			...
			while ((cur_ns - o_pre_ns) >= delay_ms*1800000LL) {
				MAG_LOG("--- o_pre_ns(%d) behind cur_ns(%d) exceed delay_ms(%d) ---\n",
						(int)o_pre_ns, (int)cur_ns, (int)delay_ms);
...
static struct mag_context *mag_context_alloc_object(void)
{
	...
	obj->is_first_data_after_enable_mag = false;  /* [yanlin] Edit for enable error */
	obj->is_first_data_after_enable_ori = false;  /* [yanlin] Edit for enable error */

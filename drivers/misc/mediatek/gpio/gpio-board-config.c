...
#define DEFAULT_BOARD_CONFIG_GPIO	116
...
#define DEFAULT_SUB_BOARD_CONFIG_GPIO	88
static unsigned sub_board_config_gpio = DEFAULT_SUB_BOARD_CONFIG_GPIO;
#endif
...
#if defined(CONFIG_TPLINK_PRODUCT_TP910) || defined(CONFIG_TPLINK_PRODUCT_TP912)
		return 1 - gpio_get_value(board_config_gpio);
#else
		...
		struct device_attribute *attr, char *buf)
{
	/* [liguanxiong] use common linux gpio api */
#if defined(CONFIG_TPLINK_PRODUCT_TP910) || defined(CONFIG_TPLINK_PRODUCT_TP912)
	return scnprintf(buf, PAGE_SIZE, "%d", 1 - gpio_get_value(board_config_gpio));
#else
...
#if defined(CONFIG_TPLINK_PRODUCT_TP910) || defined(CONFIG_TPLINK_PRODUCT_TP912)
static ssize_t sub_board_config_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	/* [liguanxiong] use common linux gpio api */
	return scnprintf(buf, PAGE_SIZE, "%d", 1 - gpio_get_value(sub_board_config_gpio));
}

static DEVICE_ATTR(sub_board_config, S_IRUGO, sub_board_config_show, NULL);
#endif

static struct attribute *board_config_attrs[] = {
	&dev_attr_board_config.attr,
#if defined(CONFIG_TPLINK_PRODUCT_TP910) || defined(CONFIG_TPLINK_PRODUCT_TP912)
	&dev_attr_sub_board_config.attr,
#endif
	NULL,
};
...
#if defined(CONFIG_TPLINK_PRODUCT_TP910) || defined(CONFIG_TPLINK_PRODUCT_TP912)
	ret = of_property_read_u32(pdev->dev.of_node, "gpio_config_sub",&sub_board_config_gpio);
	if (ret) {
		pr_err("%s: can not parse gpio from dts, use default\n", __func__);
		sub_board_config_gpio = DEFAULT_SUB_BOARD_CONFIG_GPIO;
	}
#endif
...
#if defined(CONFIG_TPLINK_PRODUCT_TP910) || defined(CONFIG_TPLINK_PRODUCT_TP912)
	ret = gpio_request(sub_board_config_gpio, "sub_board_config_gpio");
	if (ret) {
		pr_err("%s: set state failed!\n", __func__);
		return -EINVAL;
	}
...
MODULE_DESCRIPTION("TP902/TP903 BOARD CONFIG GPIO DRIVER");

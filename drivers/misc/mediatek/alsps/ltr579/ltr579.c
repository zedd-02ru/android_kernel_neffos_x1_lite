...
static int ltr579_dynamic_calibrate(void)
{
	...
	/* [yanlin start] Distinguish for 902/903/904 */
#ifdef CONFIG_TPLINK_PRODUCT_TP903
	if (noise < 100) {
		ps_thd_val_high = noise + 138;
		ps_thd_val_low  = noise + 95;
	}
	else if (noise < 200) {
		ps_thd_val_high = noise + 138;
		...
		ps_thd_val_high = noise + 139;
		ps_thd_val_low  = noise + 109;
	}
	else if (noise < 1000) {
		ps_thd_val_high = noise + 166;
		ps_thd_val_low  = noise + 131;
	}
	else if (noise < 1200) {
		ps_thd_val_high = noise + 178;
		ps_thd_val_low  = noise + 143;
	}
	else {
	...
	if (noise < 100) {
		ps_thd_val_high = noise + 106;
		ps_thd_val_low  = noise + 71;
	}
	else if (noise < 200) {
		...
		ps_thd_val_high = noise + 112;
		ps_thd_val_low  = noise + 72;
	}
	else if (noise < 600) {
		ps_thd_val_high = noise + 116;
		ps_thd_val_low  = noise + 75;
	}
	else if (noise < 1000) {
		ps_thd_val_high = noise + 123;
		ps_thd_val_low  = noise + 78;
	}
	else if (noise < 1200) {
		ps_thd_val_high = noise + 126;
		ps_thd_val_low  = noise + 79;
	}
	else {
		ps_thd_val_high = 1400;
		ps_thd_val_low  = 1300;
...
#else
	if (noise < 100) {
		ps_thd_val_high = noise + 95; /* [yanlin] edit for 902 */
		ps_thd_val_low  = noise + 65; /* [yanlin] edit for 902 */
	}
	else if (noise < 200) {
		ps_thd_val_high = noise + 80; /* [yanlin] edit for 902 */
		ps_thd_val_low  = noise + 53; /* [yanlin] edit for 902 */
	}
	else if (noise < 300) {
		ps_thd_val_high = noise + 80;
		ps_thd_val_low  = noise + 50; /* [yanlin] edit for 902 */
	}
	else if (noise < 400) {
		ps_thd_val_high = noise + 100;
		ps_thd_val_low  = noise + 70; /* [yanlin] edit for 902 */
	}
	else if (noise < 600) {
		ps_thd_val_high = noise + 120;
		ps_thd_val_low  = noise + 90; /* [yanlin] edit for 902 */
	}
	else if (noise < 1000) {
		ps_thd_val_high = noise + 150;
		ps_thd_val_low  = noise + 115; /* [yanlin] edit for 902 */
	}
	else {
		ps_thd_val_high = 1300;
		ps_thd_val_low  = 1200;
	}
#endif
	/* [yanlin end] Distinguish for 902/903/904 */
...
#ifdef CONFIG_TPLINK_PRODUCT_TP903
				if(obj->ps < 100){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 138);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 95);
				}else if(obj->ps < 200){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 138);
					...
					atomic_set(&obj->ps_thd_val_high, obj->ps + 139);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 109);
				}else if(obj->ps < 1000){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 166);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 131);
				}else if(obj->ps < 1200){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 178);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 143);
				}else{
...
#elif defined(CONFIG_TPLINK_PRODUCT_TP904)
				if(obj->ps < 100){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 106);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 71);
				}else if(obj->ps < 200){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 110);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 75);
				}else if(obj->ps < 300){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 110);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 75);
				}else if(obj->ps < 400){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 112);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 72);
				}else if(obj->ps < 600){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 116);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 75);
				}else if(obj->ps < 1000){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 123);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 78);
				}else if(obj->ps < 1200){
					atomic_set(&obj->ps_thd_val_high, obj->ps + 126);
					atomic_set(&obj->ps_thd_val_low, obj->ps + 79);
				}else{
					atomic_set(&obj->ps_thd_val_high, 1400);
					atomic_set(&obj->ps_thd_val_low, 1300);
				}
#else
				if(obj->ps < 100){
					atomic_set(&obj->ps_thd_val_high, obj->ps+95);  /* [yanlin] edit for 902 */
					atomic_set(&obj->ps_thd_val_low, obj->ps+65);   /* [yanlin] edit for 902 */
				}else if(obj->ps < 200){
					atomic_set(&obj->ps_thd_val_high, obj->ps+80);  /* [yanlin] edit for 902 */
					atomic_set(&obj->ps_thd_val_low, obj->ps+53);   /* [yanlin] edit for 902 */
				}else if(obj->ps < 300){
					atomic_set(&obj->ps_thd_val_high,  obj->ps+80);
					atomic_set(&obj->ps_thd_val_low, obj->ps+50);   /* [yanlin] edit for 902 */
				}else if(obj->ps < 400){
					atomic_set(&obj->ps_thd_val_high,  obj->ps+100);
					atomic_set(&obj->ps_thd_val_low, obj->ps+70);   /* [yanlin] edit for 902 */
				}else if(obj->ps < 600){
					atomic_set(&obj->ps_thd_val_high,  obj->ps+120);
					atomic_set(&obj->ps_thd_val_low, obj->ps+90);   /* [yanlin] edit for 902 */
				}else if(obj->ps < 1000){
					atomic_set(&obj->ps_thd_val_high,  obj->ps+150);
					atomic_set(&obj->ps_thd_val_low, obj->ps+115);  /* [yanlin] edit for 902 */
				}else if(obj->ps < 1250){
					atomic_set(&obj->ps_thd_val_high,  obj->ps+160);
					atomic_set(&obj->ps_thd_val_low, obj->ps+120);
				}
				else{
					atomic_set(&obj->ps_thd_val_high,  1300);
					atomic_set(&obj->ps_thd_val_low, 1200);
				}
#endif
				/* [yanlin end] Distinguish for 902/903/904 */
...
static int ltr579_i2c_remove(struct i2c_client *client)
{
	...
	err = misc_deregister(&ltr579_device);
	if(err)
	{

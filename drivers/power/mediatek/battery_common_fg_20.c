...
/* [lifeiping start] add screen state flag */
unsigned int g_screen_state = SCREEN_OFF;
/* [lifeiping end] */
...
kal_bool is_transport_mode = KAL_FALSE;
/* [lidebiao start] control battery level for runin test */
kal_bool is_disable_charging_mode = KAL_FALSE;
kal_bool is_in_runin_test_mode = KAL_FALSE;
/* [lidebiao end] */
/* [lidebiao start] Add sysfs interface for Disable temp protect */
kal_bool is_enable_temp_charging_protect_mode = KAL_TRUE;
/* [lidebiao end] */
kal_bool is_fastcharging_test = KAL_FALSE;	// [lidebiao] add for fastcharging test
/* ////////////////////////////////////////////////////////////////////////////// */
...
/* [lifeiping start] add for reliability test */
#define ABOVE_SAMPLE_TEMP 65
#define SAMPLE_LIMIT_TEMP 60
/* [lifeiping end] */
...
/* [lidebiao start] Detect TA repeatedly to prevent errors */
static struct workqueue_struct *ta_detection_workqueue;
static struct work_struct ta_detection_work;
/* [lidebiao end]*/
...
/* CC: modify to have a full power supply status */
	...
	.BAT_CAPACITY = -1,
#else
	.BAT_CAPACITY = -1,	// [lidebiao] Modify bat level default value
#endif
	.BAT_batt_vol = 0,
	.BAT_batt_temp = 0,
	/* Dual battery */
...
unsigned char g_exhibition_mode = 0;
static ssize_t store_exhibition_mode(struct device *dev, struct device_attribute *attr,
				    const char *buf, size_t size)
{
	...
	if (buf != NULL && size != 0) {
		pr_err("[store_exhibition_mode] buf is %s and size is %zu\n", buf, size);
		val = simple_strtoul(buf, &pvalue, 16);

		if ((val == 0) || (val == 1)) {
			g_exhibition_mode  = val;
		} else {
			pr_err("[store_exhibition_mode] Input value=%d is out of range {0, 1}\n", val);
		}
	}
	return size;
}

static ssize_t show_exhibition_mode(struct device *dev, struct device_attribute *attr,
				    char *buf)
{
	int ret_value = 0;

	pr_err("[show_exhibition_mode] g_exhibition_mode=%d\n", g_exhibition_mode);
	return sprintf(buf, "%u\n", ret_value);
}

static DEVICE_ATTR(exhibition_mode, 0664, show_exhibition_mode, store_exhibition_mode);	/* 664 */

unsigned int g_total_charger_on_time = 0;
static ssize_t show_total_charger_on_time(struct device *dev, struct device_attribute *attr,
				    char *buf)
{
	int ret_value = 0;

	ret_value = g_total_charger_on_time;
	battery_log(BAT_LOG_CRTI, "g_total_charger_on_time: %d\n", ret_value);
	return sprintf(buf, "%u\n", ret_value);
}

static DEVICE_ATTR(total_charger_on_time, 0664, show_total_charger_on_time, NULL);	/* 664 */
// [wuzhe end]
...
/* [lifeiping start]*/
static ssize_t show_Charging_ScreenState(struct device *dev, struct device_attribute *attr, char *buf)
{
        battery_log(BAT_LOG_CRTI, "Screen state = %d\n", g_screen_state);
        return sprintf(buf, "%u\n", g_screen_state);
}

static ssize_t store_Charging_ScreenState(struct device *dev, struct device_attribute *attr,
                            const char *buf, size_t size)
{
        if (kstrtouint(buf, 10, &g_screen_state) == 0) {
                battery_log(BAT_LOG_CRTI, "screen state = %d\n", g_screen_state);
                return size;
        }
...
static DEVICE_ATTR(Charging_ScreenState, 0664, show_Charging_ScreenState, store_Charging_ScreenState);

unsigned int get_Screen_State(void)
{
...
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_20_SUPPORT)
static ssize_t show_Pump_Express_20_Charger(struct device *dev, struct device_attribute *attr, char *buf)
{
	int is_ta_detected = 0;
	...
	if (STANDARD_CHARGER == BMT_status.charger_type &&
		KAL_TRUE == mtk_pep20_get_to_check_chr_type() &&
		BMT_status.UI_SOC2 < batt_cust_data.ta_stop_battery_soc) {
		msleep(200);
		battery_log(BAT_LOG_CRTI, "[%s] sleep 200ms\n",__func__);
	}

	/* [lidebiao start] add for fastcharging test */
	if ( KAL_TRUE == mtk_pep20_get_is_connect_for_charging_test() ) {
		is_ta_detected = 1;
	}

	battery_log(BAT_LOG_CRTI, "mtk_pep20_get_is_connect_for_charging_test() = %d\n", mtk_pep20_get_is_connect_for_charging_test());
	/* [lidebiao end] */

	return sprintf(buf, "%d\n", is_ta_detected);
}
static DEVICE_ATTR(Pump_Express_20_Charger, 0664, show_Pump_Express_20_Charger, NULL);

static ssize_t store_Pump_Express_20_Vbus(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	unsigned int val;
	int ret;

	battery_log(BAT_LOG_CRTI, "Pump_Express buf= %s\n", buf);
	ret = kstrtouint(buf, 10, &val);
	battery_log(BAT_LOG_CRTI, "Pump_Express ret= %u\n", ret);
	if (ret == 0) {
		ret = battery_pump_express20_algorithm_set_vbus(val);
		if (-2 == ret) {
			battery_log(BAT_LOG_CRTI, "Set Vbus failed\n");
		}
		battery_log(BAT_LOG_CRTI, "Pump_Express ret= %d\n", ret);
...
static ssize_t show_Pump_Express_20_Vbus(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int charger_vol;
	charger_vol = bq25890_get_vbus();
	//charger_vol = battery_meter_get_charger_voltage();
	battery_log(BAT_LOG_CRTI, "charger voltage:%d\n", charger_vol);
	return sprintf(buf, "%u\n", charger_vol);
}

static DEVICE_ATTR(Pump_Express_20_Vbus, 0664, show_Pump_Express_20_Vbus, store_Pump_Express_20_Vbus);

static ssize_t show_Pump_Express_20_Current(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int ICharging;
	ICharging =  bq25890_get_ichg();
	//ICharging =  battery_meter_get_battery_current();
	battery_log(BAT_LOG_CRTI, "battery current:%u\n", ICharging);
	return sprintf(buf, "%u\n", ICharging);
}
static DEVICE_ATTR(Pump_Express_20_Current, 0664, show_Pump_Express_20_Current, NULL);
#endif

static ssize_t show_Set_Transport_Mode(struct device *dev, struct device_attribute *attr, char *buf)
{
	is_transport_mode = KAL_TRUE;
	return sprintf(buf, "%u\n", is_transport_mode);
}
static DEVICE_ATTR(Set_Transport_Mode, 0664, show_Set_Transport_Mode, NULL);

/* [lidebiao start] control battery level for runin test */
static ssize_t show_Disable_Charging_Mode(struct device *dev, struct device_attribute *attr, char *buf)
{

	is_disable_charging_mode = KAL_TRUE;
	is_in_runin_test_mode = KAL_TRUE;
	battery_log(BAT_LOG_CRTI, "is_disable_charging_mode:%d\n", is_disable_charging_mode);
	return sprintf(buf, "%u\n", is_disable_charging_mode);
}
static DEVICE_ATTR(Disable_Charging_Mode, 0664, show_Disable_Charging_Mode, NULL);

static ssize_t show_Enable_Charging_Mode(struct device *dev, struct device_attribute *attr, char *buf)
{

	is_disable_charging_mode = KAL_FALSE;
	battery_log(BAT_LOG_CRTI, "is_disable_charging_mode:%d\n", is_disable_charging_mode);
	return sprintf(buf, "%u\n", is_disable_charging_mode);
}
static DEVICE_ATTR(Enable_Charging_Mode, 0664, show_Enable_Charging_Mode, NULL);
/* [lidebiao end] */

/* [lidebiao start] add for fastcharging test */
static ssize_t show_Enable_FastCharging_Test_Mode(struct device *dev, struct device_attribute *attr, char *buf)
{

	is_fastcharging_test = KAL_TRUE;
	is_in_runin_test_mode = KAL_TRUE;	// [lidebiao] Solve battery level error when high temp
	battery_log(BAT_LOG_CRTI, "is_fastcharging_test:%d\n", is_fastcharging_test);
	return sprintf(buf, "%u\n", is_fastcharging_test);
}
static DEVICE_ATTR(Enable_FastCharging_Test_Mode, 0664, show_Enable_FastCharging_Test_Mode, NULL);

static ssize_t show_Disable_FastCharging_Test_Mode(struct device *dev, struct device_attribute *attr, char *buf)
{

	is_fastcharging_test = KAL_FALSE;
	mtk_pep20_set_is_enable(true);
	mtk_pep20_set_to_check_chr_type(true);
	battery_log(BAT_LOG_CRTI, "is_fastcharging_test:%d\n", is_fastcharging_test);
	return sprintf(buf, "%u\n", is_fastcharging_test);
}
static DEVICE_ATTR(Disable_FastCharging_Test_Mode, 0664, show_Disable_FastCharging_Test_Mode, NULL);
/* [lidebiao end] */

/* [lidebiao start] Add sysfs interface for Disable temp protect */
static ssize_t show_Enable_Temp_Charging_Protect_Mode(struct device *dev, struct device_attribute *attr, char *buf)
{

	is_enable_temp_charging_protect_mode = KAL_TRUE;
	return sprintf(buf, "%u\n", is_enable_temp_charging_protect_mode);
}
static DEVICE_ATTR(Enable_Temp_Charging_Protect_Mode, 0664, show_Enable_Temp_Charging_Protect_Mode, NULL);

static ssize_t show_Disable_Temp_Charging_Protect_Mode(struct device *dev, struct device_attribute *attr, char *buf)
{

	is_enable_temp_charging_protect_mode = KAL_FALSE;
	return sprintf(buf, "%u\n", is_enable_temp_charging_protect_mode);
}
static DEVICE_ATTR(Disable_Temp_Charging_Protect_Mode, 0664, show_Disable_Temp_Charging_Protect_Mode, NULL);
/* [lidebiao end] */

/* [zhangdong start] Update interface for disable temp protect */
...
{
	int ret;
	unsigned int batt_temp_protect_mode = 1;
...
/* [lifeiping start] */
kal_bool get_Temp_Charging_Protect_Mode(void)
{
	return is_enable_temp_charging_protect_mode;
}
/* [lifeiping end] */

kal_bool is_set_transport_mode()
{
	return is_transport_mode;
}
...
{
	/* [lidebiao start] control battery level for runin test */
	if (is_in_runin_test_mode == KAL_TRUE) {
...
{
/* [lidebiao start] Add sysfs interface for Disable temp protect */
	if (is_enable_temp_charging_protect_mode == KAL_FALSE ) {
		return;
	}
/* [lidebiao end] */
...
{
	/* [lifeiping start] add threshold judge for reliability test */
	unsigned int temp_protect_threshold;

	if (KAL_FALSE == is_enable_temp_charging_protect_mode) {
		temp_protect_threshold = ABOVE_SAMPLE_TEMP;
	} else {
		temp_protect_threshold = SAMPLE_LIMIT_TEMP;
	}
	/* [lifeiping end] */
	...
	if ((g_battery_thermal_throttling_flag == 1) || (g_battery_thermal_throttling_flag == 3)) {
		...
		/* ignore default rule */
#else
		/* [lifeiping start] Modify temp protect threshold */
		if (BMT_status.temperature >= temp_protect_threshold) {
		/* [lifeiping end] */
#if defined(CONFIG_POWER_EXT)
			battery_log(BAT_LOG_CRTI,
...
/* [lidebiao start] Detect TA repeatedly to prevent errors */
static void do_ta_detection_work(struct work_struct *work)
{
	unsigned int count = 0;
	battery_log(BAT_LOG_CRTI, "enter do_ta_detection_work!\n");
	while (BMT_status.charger_type == NONSTANDARD_CHARGER) {
		msleep(2000);
		battery_log(BAT_LOG_CRTI, "start ta detection!\n");
...
*/
	unsigned int pwr;
#endif
	int ret;	// [lidebiao] Detect TA repeatedly to prevent errors
#ifdef CONFIG_TCPC_CLASS
	...
	BMT_status.charger_vol = battery_meter_get_charger_voltage();
		...
		if ((BMT_status.charger_type == CHARGER_UNKNOWN) &&
			...
			/* [lidebiao start] Detect TA repeatedly to prevent errors */
			if (BMT_status.charger_type == NONSTANDARD_CHARGER) {
				ret = queue_work(ta_detection_workqueue, &ta_detection_work);
				if (!ret)
					pr_err("ta_detection_work failed\n");
			}
			/* [lidebiao end] */
...
/*[lidebiao start] for compatible with 5V2A TA*/
			...
			battery_log(BAT_LOG_CRTI,
				    "[do_chrdet_int_task] is_5v2a_ta_connect:%d, charge_current_check_count:%d\n", is_5v2a_ta_connect, charge_current_check_count);
...
#endif
			is_disable_charging_mode = KAL_FALSE;	// [lidebiao] disable charging for control battery level
			is_fastcharging_test = KAL_FALSE;	// [lidebiao] add for fastcharging test
		}
...
void BAT_thread(void)
{
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (battery_suspended == KAL_FALSE)) {
		...
		g_total_charger_on_time += BAT_TASK_PERIOD; // [wuzhe] Add for exhibition mode support
	} else {
		g_total_charger_on_time = 0; // [wuzhe] Add for exhibition mode support
	}
...
static int battery_probe(struct platform_device *dev)
{
	...
	/* For EM */
	{
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charging_CallState);
                /* [lifeiping start] */
                ret_device_file = device_create_file(&(dev->dev), &dev_attr_Charging_ScreenState);
                /* [lifeiping end] */
		...
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Set_Transport_Mode);
		/*[lidebiao start]*/
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Disable_Charging_Mode);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Enable_Charging_Mode);
		/*[lidebiao end]*/
		/* [lidebiao start] Add sysfs interface for Disable temp protect */
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Enable_Temp_Charging_Protect_Mode);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Disable_Temp_Charging_Protect_Mode);
		/*[lidebiao end]*/
		/* [lidebia start] add for fastcharging test */
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Enable_FastCharging_Test_Mode);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Disable_FastCharging_Test_Mode);
		/* [lidebiao end] */
#if defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_20_SUPPORT)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Pump_Express_20_Current);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Pump_Express_20_Vbus);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_Pump_Express_20_Charger);
#endif
		// [wuzhe start] add exhibition mode support
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_exhibition_mode);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_total_charger_on_time);
		// [wuzhe end]
	}
...
static int mtk_tpcharger_setup_files(struct platform_device *pdev)
{
	...
	if (ret)
		pr_err("%s: devices create file error!\n", __func__);
...
static int mtk_tpcharger_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		pr_err("%s: mtk tpcharger probe failed!\n", __func__);
		return ret;
	}
...
static int __init battery_init(void)
{
	...
	if (ret) {
		battery_log(BAT_LOG_CRTI,
			    "****[tpcharger_driver] Unable to register driver (%d)\n", ret);
		return ret;
	}

	ret = platform_device_register(&tpcharger_device);
	if (ret) {
		battery_log(BAT_LOG_CRTI,
			    "****[tpcharger_device] Unable to register device (%d)\n", ret);
		platform_driver_unregister(&tpcharger_driver);
		return ret;
	}
	/* [zhangdong end] */
	...
	/* [lidebiao start] Detect TA repeatedly to prevent errors */
	ta_detection_workqueue= create_singlethread_workqueue("neffos-ta-detection");
	INIT_WORK(&ta_detection_work, do_ta_detection_work);
	/* [lidebiao end] */

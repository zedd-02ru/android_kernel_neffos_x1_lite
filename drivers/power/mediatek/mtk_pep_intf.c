...
static bool pep_reset_5v_for_temp = false;	// [lidebiao] Charging too long for high temp
...
int mtk_pep_reset_ta_vchr_for_temp(void)
{
...
void mtk_pep_set_is_cable_out_occur(bool out)
{
	...
	pep_reset_5v_for_temp = false;	// [lidebiao] Charging too long for high temp
	mutex_unlock(&pep_pmic_sync_lock);
}

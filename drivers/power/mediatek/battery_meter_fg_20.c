...
int __batt_meter_init_cust_data_from_dt_part(struct platform_device *dev)
{
	...
	while (!of_property_read_u32_index(np, "batt_temperature_table", idx, &addr)) {
		...
		/* [lidebiao start] Solve Compile error for merging upstream code */
...
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
/* [lidebiao start] compatible multi bat for TP903 */
...
int BattThermistorConverTemp(int Res)
{
	...
	int RES1 = 0, RES2 = 0;
	int TBatt_Value = -200, TMP1 = 0, TMP2 = 0;
	bm_notice("BattThermistorConverTemp Res =%d\n", Res);
	if (Res >= Batt_Temperature_Table[0].TemperatureR) {
		TBatt_Value = -20;
	} else if (Res <= Batt_Temperature_Table[17].TemperatureR) {
		TBatt_Value = 65;
	...
	bm_notice("BattThermistorConverTemp TBatt_Value = %d\n", TBatt_Value);
	return TBatt_Value;
}
#else
int BattThermistorConverTemp(int Res)
...
/* [lidebiao start] add temp strategy for TP903 */
#if defined(CONFIG_TPLINK_PRODUCT_TP903)
	if ( bat_temperature_val > 4) {
		bat_temperature_val = bat_temperature_val - 3;
	} else if ( bat_temperature_val == 4) {
		bat_temperature_val = bat_temperature_val - 2;
	} else if ( bat_temperature_val == 3) {
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_GET_BATTERY_INIT_VOLTAGE:
		{
			/* [houjihai start] Fix read init voltage and current failed */
			int voltage_init_try_num = 0;

			while (gFG_voltage_init == 0) {
				msleep(100);
				bm_err("FG_DAEMON_CMD_GET_BATTERY_INIT_VOLTAGE waiting ,%d %d\n",
				gFG_voltage_init, gFG_current_init);
				voltage_init_try_num ++;
				if (voltage_init_try_num >= 20)
					break;
			}
			ret_msg->fgd_data_len += sizeof(gFG_voltage_init);
			memcpy(ret_msg->fgd_data, &gFG_voltage_init, sizeof(gFG_voltage_init));

			bm_err("[fg_res] voltage = %d gFG_current_init = %d voltage_init_try_num = %d\n", gFG_voltage_init, gFG_current_init, voltage_init_try_num);
			/* [houjihai end] */
		}
		break;
	...
	case FG_DAEMON_CMD_IS_HW_OCV_UPDATE:
		...
		case FG_DAEMON_CMD_SET_CAR_TUNE_VALUE:
		{
			...
			bm_err("[fg_res] cali_car_tune = %d(%d), default = %d, Use[cali_car_tune]\n",
...
static void nl_data_handler(struct sk_buff *skb)
{
	...
	if (fgd_msg->fgd_ret_data_len >= FGD_NL_MSG_MAX_LEN) {
		bm_err("nl_data_handler err: cmd:%d len:%d ret_data:%d\n",
			fgd_msg->fgd_cmd, fgd_msg->fgd_data_len, fgd_msg->fgd_ret_data_len);
		return;
	}
